...
**WAVE 0 — DELIVERED 2026-09-11, founder sending by hand as the session closed (`audit/recipients/wave0/SEND_SHEET_2026-09-11.md`, gitignored).** Six fresh full-tier re-runs (90p; one duplicate from a transport retry). Two of the three fresh records for the old recipients were WORSE for their readers and were held (NHS: causation badged supported on two pages from one site incl. a tag index; Scotland: the contested £53m merged away, Breitbart on top), so the send set is: **TTE → `1bbf008a` (2 Sep, B) · Viglione → `441144ac` (31 Aug, B; her factcheck in neither fresh pool — the Phase D structural finding again) · Seymour → `580fd5b4` (new, B−) · Tapper → `4be28cd1` (2 Sep, B−) · McSweeney → `bebfa026` (new, B).** Every note pressure-passed (page, every lens, PDF) and fact-checked sentence-by-sentence by a second agent; **two rewritten on real errors** (McSweeney misquoted the Telegraph headline; Tapper's DM attributed the £53m-as-forecast framing to him when his own chart rejects it). **FIRST TOUCH — Dr Giuliana Viglione (Carbon Brief) · 2026-09-16 16:24 BST · Carbon Brief contact form · record `441144ac` (31 Aug, B, her factcheck filed as the challenge on both parts) · `src=o-viglione`.** The first send of the whole campaign. Earlier the same day the TTE email reply bounced (Substack: paid subscribers only) — TTE goes by Substack DM next. **Second send: Richard Seymour · 2026-09-16 16:43 BST · Substack DM · record `580fd5b4` (11 Sep, B−) · `src=o-seymour`.** **WAVE 1 — Monday's 25 topics verified 25/25 (`audit/recipients/wave1/2026-09-14_topics.md`)**; records not yet run (needs the founder's go, ~£3.75 nominal); expect ~half held after the pressure pass; three rows are rebuttals (Dunt, Legum, Morris) that the record must find. Sends of that batch land TUESDAY at the queue's slots. Verdict read **Mon 21 Sep**.
