...
## 9. Build 3 premise check (founder asked for verification before approving) — the "≈4 points, mechanical" claim was OVERSTATED

Script: `tmp/verify-build3-2026-09-10.txt` (all 722 labels across the six runs; every rejected AND every justified passage tagged by candidate lexical signature; stage of origin from the stored evidence).

- **Only 12 of 83 rejections carry any lexical signature**, and two of the candidate signatures (`fragment` = starts mid-sentence, `very_short`) also match **20 justified labels** — unusable as a guard. The SAFE signatures — a rating badge alone, site chrome ("Notifications You must be signed in…", "Our Services Medical Information…"), a bare citation ("Cite chart IEA (2021)…"), a definition ("Inflation rate is the percentage increase…") — match **6 rejections and 1 justified label** (the same ubiehealth chrome window, justified once, rejected once).
- **Origin of those 6:** the AFP badge (2, already fixed by build 2's adapter change — 0 AFP rejections on the b2 runs); GitHub and ubiehealth site chrome (2, `contentBasis=full/snippet`); the IEA citation block (1, `contentBasis=full`); the calculatorlib definition (1, `contentBasis=distilled`). The NEJM "2% among patients…" fragment (3, `contentBasis=pdf`) is a PDF window starting mid-sentence — no safe signature.
- **The other 71 rejections have no signature.** They are the mapper reading a same-subject passage as the finding (the WAL-mode passages for "SQLITE_BUSY cannot occur", the Venus temperatures, the trial-design passages, the Sweden "EU and Nordic" for "every European country") and the reviewer's strictness / errors. Prompted already (FINDING, NOT TOPIC); no lexical form; further prompt tweaks are unmeasurable in ±3-point pool noise.

**Corrected reading:** a window guard on the four safe signatures recovers **≈6 labels / 722 ≈ 0.8 points**, not four. The "≈4 points ours" figure in §8 counted every topic-without-finding label as if it had a mechanical form; most do not. The reachable improvement from a build 3 is therefore about one point blind (≈90–91%), lifting adjudicated to ≈95%. The rest of the blind gap is split between the user's own wording (≈3.6 points, never soften) and reviewer error/strictness (≈1 point) and the mapper's same-subject reading (≈2 points, prompt-only, measurable only).

**Reduced proposal for build 3, if wanted:** a window guard at content selection — drop a candidate window that is a rating badge alone, site chrome, a bare citation, or a definition sentence, and take the next window (or leave the item unmapped with a receipt). ~30 minutes, ~1 point, one justified label at risk. Worth doing for the record's cleanliness; it does not close the bar on its own, and nothing left in the data does.
