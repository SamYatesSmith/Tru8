...
---
## 🟢 START HERE — next session (updated 2026-09-02, sign-off)

**This block is what to do next. Everything below the divider is history.**

### ▶▶ WHERE 2026-09-02 ENDED — five things shipped and live; send week is THREE, not five; Build A awaits its proof.

**Live on production today (all verified live, all on main):**
- **Fix 1** `46efd58` — element caveats render on `/r/` as a grey `NOTE ·` row behind a fail-closed no-verdict gate (`web/lib/element-caveat.ts`). Design: `audit/2026-09-02_fix1_element_caveat_render_design.md`.
- **iOS tap-zoom** `3ea7d06` — 16px floor on form fields for touch devices; the homepage field explicit.
- **Playwright** `cb6711a` — `cd web && node scripts/verify-page.mjs <url> --device "iPhone 15"`; the page-verification tool from now on. Chrome extension only for Clerk-walled flows.
- **Compact phone cookie banner** `0cb41b1` — 164px strip, was ~55% of the first screen.
- **Build A** `adf252d` — the claim lane's lead query now also runs UNWINDOWED (`ENABLE_CLAIM_LANE_UNWINDOWED_TWIN`). Why: the past-month window returned only blocklisted social links, so the lane contributed nothing and critics appeared by luck. Design: `audit/2026-09-02_claim_lane_unwindowed_twin_design.md`; evidence: `audit/2026-09-02_dissent_discovery_probe.md`.

**Send sheet** (`audit/2026-08-21_send_sheet.md`, untracked): all notes re-verified against the live pages and rewritten for Fix 1. **McSweeney re-run `e1e5de25`** (carbonbrief.org now in the pool, B+). **TTE HELD** — a 15p re-run lost their own piece from the pool. **Tapper held** by the founder's standard (C+). **Send THREE: Viglione, Seymour, McSweeney.**

**Decisions today:** headline grey KEPT · Option B (signed-out runs) = capped subsidy, measure first (logged in the front-door review §4B) · Brave is on the FREE plan (2,000/month) so Build B is cheap, not free.

### ▶▶ WHAT HAPPENS NEXT, in order:
1. **Control arm for Build A — RUN 2026-09-02 14:03/14:11 on the founder's go (2 × 15p). SCOTLAND PASSES; TTE WAS VOID.**
   - **Scotland `4be28cd1` — PASS, and a real test:** Railway logs show the claim lane's lead (`num=13 tbs=qdr:y`) AND its twin (`num=10 tbs=none`, same claim text) going out live; 46 web searches. `futureeconomy.scot/587` is in the pool, shown, mapped `e1: challenges`, `e3: context`. The record's shape is now what Macfarlane argued: **e1 "the £53m forecast" DISPUTED** (three sources: the SFC's post-behaviour forecast was £8m), e2 £22m loss supported (3), e3 causation unresolved (a Reddit recital scoped). Tapper's old record `5d69fc71` read supported ×3. Tapper's note needs a full rewrite against this record's rendered page + PDF before it can go.
   - **TTE, second attempt after Piece 3 (15:40, founder: "Proceed on the control run") — Build A RAN, the critic's page WAS FETCHED, and a pre-existing 45-second deadline threw the whole web lane away.** Logs for `dd2ca726` (request `32a4179b`): `Query lanes … per_lane={'c0': 4, …}` with the twin visible (`num=10 tbs=none`), `Fetch budget candidates=48 fetched=40`, then `[DOMAIN_TRACKER] trusttheevidence.substack.com -> accessible` at 15:40:24 — **and at 15:41:02 `[CLAIM 0] Tasks timed out after 45s: ['web_search']` → `0 web snippets + 1 API snippets`.** `retrieve.py:1751` `RETRIEVE_CLAIM_TIMEOUT_S=45` wraps search+fetch of all 40 URLs as ONE task; the code comment says "preserve partial results" but that is only between the web and API tasks — inside the web task it is all-or-nothing, so every fetched page is discarded at the deadline and the pool is rebuilt by post-filter recovery + coverage recovery (windowed, 8 results). **That is why every TTE run comes out recovery-shaped and thin, and why Build A cannot yet be proved on this claim.** Scotland finished inside 45 s (30 candidates); TTE had 48 with slow Substack/PDF/doi fetches. **Defect to fix, scoped not built: the web task must return what it has at the deadline (collect completed extractions; cancel only the stragglers).** ⚠️ **Second finding: ONE tool call produced TWO charged checks (`dd2ca726` 15:40 and `c8dd4886` 15:42, 15p each).** The first ran fresh and wrote the cache under the new key (`[CACHE OK] … version=2026-09-02a` — Piece 3 verified live); **the HTTP log shows the mechanism, and it is NOT specific to this client:** the hosted MCP's streamable-HTTP `GET /mcp/` stream dies at ~140 s (every one logs `200 140,1xx ms`), after which the client's next `POST /mcp/` gets `400` (session gone), it re-initialises and **re-sends the pending tool call**, and the server executes it again (bursts of `400 → 200/202/200` at 15:38:53, 15:40:09 and 15:41:27 bracket the two checks). Any hosted-MCP user running a full-tier check that outlives ~140 s will be double-charged the same way. The tool re-POSTed `/agent/check` with NO `Idempotency-Key` (the API supports one; `tru8_mcp/tools.py` never sends it), so the retry became a new check, hit the fresh cache (`web_search=0`), and its replay is what the tool returned. **Fix, small: the MCP server derives an Idempotency-Key per tool invocation.** Founder was charged 30p for this arm, not 15p. **▶ BOTH FIXED the same evening (founder: "Proceed with recommendation") — `audit/2026-09-02_fetch_deadline_and_mcp_idempotency.md`.** (1) `ENABLE_FETCH_PHASE_DEADLINE` / `RETRIEVE_FETCH_PHASE_TIMEOUT_S=30`: fetches run as tasks under their own deadline, finished pages are KEPT, stragglers cancelled with ledger receipts (`stage=fetch_deadline`). (2) `tru8_mcp/tools.py` sends `Idempotency-Key` (endpoint + payload + 10-min window) on every agent POST, and `agent.py::_run_agent_pipeline` now returns the ORIGINAL check when `charge()` hands back a transaction that already owns one (waits if in flight; `chargedPence 0`; `X-Tru8-Idempotent-Replay: 1`) — before this, a reused key skipped the charge but still ran a second pipeline and re-pointed the transaction. `RETRIEVAL_CACHE_VERSION` → `2026-09-02b`. Tests: +5 deadline, +6 key, +5 replay; agent/MCP/services suites green. **✅ TTE CONTROL ARM PASSES — `1bbf008a`, 16:19, one check, 15p (founder: "Push it and run the TTE control arm once it's live").** Logs: `Query lanes … c0: 2` (lead + twin), `Fetch budget candidates=30 fetched=30`, **`All tasks complete for claim 0`** (no 45 s cut), `18 web snippets`, `[URL LEDGER] claim=0 kept … trusttheevidence.substack.com/p/the-miracle-statistic-revisited`, scorer 5/5, `[CACHE OK] … version=2026-09-02b`, `web_search=19`, wall 31.9 s; **exactly one check created** (the idempotency fix held — no retry, no double charge). **Their piece is IN the pool, shown, mapped `context` on e3 (causation) and e4 (the 29%)** — and the record now says their point in its own caveats: e3 causation **UNRESOLVED** (*"an internal NHS England pre/post evaluation comparing two five-month telephony periods rather than a controlled causal study"*), e4 note *"The 29 percent metric comes exclusively from internal NHS England telephony evaluations of one practice"*, e1/e2 supported with single-practice caveats. 19 reviewed / 14 organised / 13 domains. **Build A + the fetch deadline + the idempotency fix are all proved live on the claim that motivated them. Both held sends (TTE on this record, Tapper on `4be28cd1`) can now be rewritten against the rendered page + PDF and sent — founder's call on timing.** Still owed: the bench re-record (Build A + deadline re-keyed every wired cassette; ~£0.80).
   - **TTE `3cc642a4` (first attempt, 14:03) — VOID, not a fail: retrieval never ran.** `[PIPELINE METRICS] web_search=0 api_adapters=0`, retrieve stage 3 s. `app.workers.pipeline`: *"[CACHE HIT] Claim 0: retrieved 14 cached evidence items … Cache check: 1 cached, 0 uncached"* and *"Using cached claim extraction"* — the per-claim EVIDENCE CACHE (populated by `b0398fca` at 11:12) served the pool wholesale, junk included (rabbitmq.com/docs/queues, x.com/QueuesPub, later excluded by the scorer). The only live searches were coverage recovery: four, all `tbs=qdr:m`, `num=8`. **`max_age_hours=0` bypasses the analysis lookup, NOT this cache.** ⚠️ **Durable: a re-run within the evidence-cache TTL is a replay — it cannot test a retrieval change, and any "different answer each run" reading must first check `web_search=` in the metrics line.** TTE's control arm still needs a run that misses the cache: **key `tru8:evidence_extract:<md5(claim text)>`, TTL 24 h** (`cache.py` `ttl_config`); purge with `railway ssh -- python -c "…CacheService().delete('evidence_extract', md5)"` (no cost) then re-run the identical text (15p) — or wait until after 11:12 on 2026-09-03. Also noticed: **coverage recovery windows every query to the past month (`tbs=qdr:m`, `num=8`) with no unwindowed twin** — the same defect one stage later; and **the 24 h evidence cache means a retrieval improvement does not reach a repeated claim for a day** (product question: should the cache key carry the retrieval version/flags?). Both logged as follow-ups, not built. **→ SCOPED same day at the founder's request: `audit/2026-09-02_pool_quality_gate_scope.md` — "cache as a floor, not a ceiling": (3) version the cache key + 1 h TTL for breaking news [1 h]; (2) recovery searches the claim's own words unwindowed + F1-D3 hedge [2 h]; (1) recovery trigger reads pool QUALITY (one-sided AND single-original/thin, or junk-heavy) not just quantity, with a receipt [half a day] — measure the trigger rate on stored records before flipping. ~1 day total, order 3→2→1, then a 2 × 15p control arm. **▶ PIECE 3 BUILT 2026-09-02 on the founder's go ("Go on Piece 3 only, I'll send the three"):** `RETRIEVAL_CACHE_VERSION` ("2026-09-02a") is now part of the evidence-cache key (`cache.py::_evidence_identifier`), and a pd/pw claim is cached 1 h not 24 (`workers/pipeline.py::_evidence_cache_ttl`). **Deploying it invalidates every cached pool at once** — Build A reaches repeated claims immediately, and TTE's control arm can be re-run WITHOUT touching Redis by hand. Rule in CLAUDE.md: bump the version in the same commit as any retrieval change. 6 tests; runner/worker/recovery suites green; the 11 `test_cache_monitoring` failures are the documented Redis-down set (identical on main). Pieces 1+2 HELD behind the sends. **Rule VERIFIED on nine stored records (no spend) — the first draft was wrong twice (a "≤2 domains" rule fired on Viglione's two independent challengers; a "% dropped" junk rule fired on normal filtering) and both were corrected; the corrected rule fires on every TTE shape, never on Viglione/Seymour/McSweeney-original, and MISSES Tapper because the echo detector cannot see recitals of a commentary original (separate small fix to the derivation pass, scoped in the doc).**
2. **Founder go — bench re-record (~£0.80, ~10 min, docker up):** `python scripts/replay_bench.py --all --update-golden` — Build A re-keyed every wired cassette; the bench reads as drifted until then. Keep the 018F/0005 must-fire pins (merge by hand, as on 2026-08-27).
3. **Founder sends three by hand** (Viglione, Seymour, McSweeney) — read each `/r/` page first; tell the agent who/when so first-touch is logged in `audit/OUTREACH.md`.
4. **Founder phone re-test** on a FRESH tab after the iOS fix + banner; screenshot if the page still opens scrolled.
5. **After a few days:** read the front-door numbers (`claim_field_submit` vs `start_check_click`) → Option B decision. Unchanged.
6. **Small honesty fix, cheap:** the GAPS lens must stop saying "well covered" when the pool holds no challenge and the supports trace to one original (parked gap #3 from 2026-08-24).
7. Item 7 stage 2 / item 8 Option A — unchanged, after the above.

**Durable lessons written to memory today:** a model-written free-text field means nothing until the prompt defines it — read the stored corpus for verdict language before rendering it · a guarantee written for "every lane's second query" silently misses a one-query lane · when a query's results are all blocklisted the lane contributes nothing — check what a query RETURNS before theorising about ranking · re-run once before sending to anyone likely to try the claim themselves · record the parameters of every probe · iOS zooms on any control under 16px and keeps the zoom on the tab.
