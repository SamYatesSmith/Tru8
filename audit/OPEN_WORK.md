...
> **2026-09-08 — Explicit date-anchor gate implemented; broader acceptance remains open.** Initial mapping, recovery and passage review now keep day-scoped evidence as context when retained source text lacks an explicit temporal anchor. The EVIDENCE/GAPS notice explains exclusions. A full 64-arm sweep plus an 8-arm planned-change correction now passes 27 targeted conditions across 13 cases; retained quotations: 29 exact, zero invalid. Source-date/quantity rules are general, with no maintained facts or model change. Both rollout switches remain OFF: this narrow English-date/lexical check is not semantic entailment proof. Backend 97 checks, web suite 173 plus 7 final affected checks and TypeScript passed; final default replay matches 185 ok / 1 warn / 13 known fail / 2 unexercised, zero cassette drift. Next: broader unseen-source and temporal-language acceptance before activation, then the remaining original pipeline/output-quality steps. Details: [date-anchor gate evaluation](2026-09-08_fact_anchor_gate.md).
