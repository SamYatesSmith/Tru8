...
1. **Ask the founder how sign-in behaved.** He saw a 500 on Google sign-in right after the deploy; Sentry, both Railway HTTP logs (no 5xx in the hour) and the web container restart all point at the deploy window or Clerk's hosted step, not our code. Unconfirmed either way.
