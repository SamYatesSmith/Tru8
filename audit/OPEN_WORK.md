...
> **2026-09-08 — Integrated evaluation complete; rollout held on measured failures.** 16 fixed-source scenarios × 4 configurations (64 arms), using unchanged configured models and frozen classifications. Live testing exposed and fixed a conflicting passage-response schema; 40 retained quotations now validate exactly. SQLite coverage improves in this sample, but page-clock temporal errors and a false cross-element challenge remain. Both rollout switches stay OFF. Next: general temporal-applicability and element-specific entailment controls, then rerun the saved failures. Details: [evaluation findings](2026-09-08_passage_factorial_evaluation.md). Backend 42 checks and default replay baseline passed; no deployment.
