...
> **✅ MERGED AND DEPLOYED 2026-09-09 ~16:53 UTC — `main` = `247a6f8` (fast-forward, 52 commits), pushed by Claude on the founder's explicit instruction.** Railway built and serves it: health `247a6f8`/production; `alembic current` → `text_provenance (head)`; `research_operation` + `report_revision` tables, `evidence.text_provenance` column and `ux_usage_events_check_kind` all confirmed in the production DB; MCP preflight 200; revision-verify route 200. Both candidate flags OFF. **Owed: one live check end to end (Bank Rate 3.75%) + one strengthening, read from the stored record and the Railway logs.**

**Where it stands (end of 2026-09-09).** Astra's plan (`tmp/tru8-hands-on/tru8-route-to-eight.md`) has nine PR steps. **Steps 1–8 are built** on local branch `codex/evidence-quality` (34 commits; unpushed; nothing deployed; `main` unmoved at `47664a1`, so the merge is a fast-forward). Both candidate flags OFF; one new default-on gate (same-study). Latest evidence on `da9b61c`: backend **3,941 passed / 0 failed / 69 skipped**, web **195 passed**, `tsc` clean, default replay **185 / 1 / 13 / 2, zero drift**, `alembic heads` = `text_provenance` (single head), both new models exported for the `create_all` bootstrap. Three paid runs today: fixed-source 39/42 → 42/42 (~4p each) and integrated run 3 (three live reports, ~13p, over estimate). Day record: [takeover](2026-09-09_takeover_stabilise.md) → [guard](2026-09-09_quantitative_result_guard.md) → [same-study gate](2026-09-09_same_study_scope_gate.md) → [run 3](2026-09-09_integrated_run3.md) → [PDF export](2026-09-09_pdf_passage_export.md). Status doc, latest checkpoint first: [`2026-09-07_evidence_quality_implementation.md`](2026-09-07_evidence_quality_implementation.md).
...
2. **Candidate fidelity (step 7 remainder).** ▶ IN PROGRESS. The frozen pair is mechanically closed by the [quantitative result guard](2026-09-09_quantitative_result_guard.md) (flag-only; unit-tested through the real parser; fingerprint v11) — paid run 1 (approved) scored **39/42**: the pair and all result controls passed; the three misses were null-result CHALLENGES demoted on a model `mismatch/result`, now closed by a second mechanical rule (a contrary result IS the challenge). paid run 2 (approved) scored **42/42**, ~4p (55k in / 8k out / 4.5k thinking). Scope review now runs as concurrent calls of ≤6 pairs so one slow call cannot lose every pair (Venus). Study independence: the [same-study scope gate](2026-09-09_same_study_scope_gate.md) (DEFAULT-ON, `ENABLE_SAME_STUDY_SCOPE_GATE`) makes hosts of one DOI/PMID/PMC count once per side. **Integrated run 3 (approved, paid) on the final phase-2 code: all three reports completed, owner/public identities agree, every SELECT support quotes the 20% figure, creatine reads `contextual` not `disputed`, the split review ran on SELECT** — [record](2026-09-09_integrated_run3.md). **Phase 2 CLOSED on the code side; the human review of relationships is still the 8/10 gate.** The pair was frozen as: `backend/tests/evaluation/passage_quality/qualitative_effect_failures.json` — two SELECT sources labelled support for the complete 20% effect when their blocks establish only a qualitative MACE benefit. Complete-assertion grounding must need amount + measure + endpoint together; preserve equivalent-measure and true-null controls. Then source-role / study-independence grouping (alternate hosts of one study are one study). Runner: `backend/scripts/evaluate_result_fidelity.py` — **paid; ask first**.
3. ✅ **Passage-backed PDF export (step 8, founder-scoped to export only, 2026-09-09).** Validated quotes and scope receipts print under each element; verdict-language rule parity-locked to the web ([record](2026-09-09_pdf_passage_export.md)). The JSON is the check response and already carried everything.
4. ▶ **Blind AI label review RUN 2026-09-09 (no human available): 86.1% justified on 137 labels, ≈88–90% adjudicated — BELOW Astra's 95%.** Misses cluster in decomposition wording (an invented "exactly 5g"; over-precise elements) plus 4 genuine mislabels; `backend/scripts/review_labels.py` is the repeatable verify step (~2p/run). ✅ Built the same night: invented-precision strip at decomposition (`precision_stripped` receipt) and a day-level `date_scope` gate (eighth gate, after measure). Two of the four genuine mislabels have no mechanical form (inflation false-proposition support, Mercury-only comparison). **Post-fix run: 81.6% on 147 labels — DOWN, on a different pool (±5 pts is retrieval noise).** 11 of 27 rejections are the EV record's grounds QUESTIONS demanding quantities the claim never stated; 7 more are new over-specifying words ("consistently", "quantified", "verified", "completely") — list widened; the `date_scope` gate over-fired on an "as of" date and was narrowed to event dates. Bench end of day: 0003/0004 re-recorded and replay-verified (~20p); **018F is also re-keyed by the same strip ("specifically" on e1) — re-record DEFERRED until after the decomposition-specificity prompt work, which re-keys everything again; record once then.** Pass state `117/4/10 + 3 drift` (82CF, 5647 random; 018F owed). **The one lever left is decomposition specificity: a prompt rule on the decomposer AND the grounds question-builder (`opinion_symmetry`) that elements/questions must not demand figures or absolutes the claim did not state — re-keys cassettes; then regrade + review again.** The sheets remain for a human if one ever becomes available: `audit/review_sheets/2026-09-09/` — 14 sheets, 137 directional labels from the final regrade, one CSV to tally, protocol in its README** (generator `backend/scripts/build_review_sheets.py <run dir> <out dir>`). Astra asked for ≥200: a second default-arm run of ~6 more inputs (~30p) closes the gap, or run the 137 first. Reviewer: a journalist / policy researcher / paralegal from the outreach list, two to three hours, not the founder. Then: reach 8/10 on the default path, per the [regrade](2026-09-09_astra_regrade.md): (a) re-verify the extraction restore on #2/#4/#6 (paid, ~15p — ask); (b) absence-of-evidence challenges (creatine: "not enough evidence" statements counted as `challenges`) — the candidate's scope review handles this, the default path needs a mechanical form; (c) ✅ question-shaped elements now read Addressed / Contested / Context only / Open on every badge and in the PDF (parity-locked table); (d) ✅ source exclusion narrowed to the submitted PAGE (founder decision); domain-mates eligible + tagged `same_domain_as_source`; `RETRIEVAL_CACHE_VERSION=2026-09-09`; (e) ✅ decomposition prompts: no trivially-true prerequisite elements (founder-approved; re-keys every cassette → full corpus `--record` approved and owed). **Candidate before activation:** narrow `fact_applicability` (a source stating the value with a date inside the window must count), fix per-element passage binding, then a fresh two-arm regrade. Then step 9. Founder-owned gates: human review of ≥200 directional relationships (≥95% justified target), 3–5 user usability tasks, and tracing Astra's three stalled checks (`10f35558…`, `6b70b074…`, `1eb6c8af…`) through Railway logs. Claude-owned: reliability runs across input paths and fault scenarios, review-timeout completeness (Venus final review hit its 25 s bound).
5. **Merge — founder's call on timing ("later, once complete, tested and working").** Checklist, every line already green on `da9b61c`: whole backend suite · web suite · `tsc` · bench 185/1/13/2 zero drift · `alembic heads` single head `text_provenance` · both new models exported for bootstrap · flags OFF · docs current. Mechanics: `git checkout main && git merge --ff-only codex/evidence-quality && git push` (fast-forward; no merge commit). ⚠️ Two migrations ship (`research_operations`, `text_provenance`) and run via `entrypoint.sh` on deploy; verify `railway ssh "python -m alembic current"` → `text_provenance (head)` afterwards. ⚠️ First live firing of the same-study gate and the first strengthening under the new operation path should be read in the Railway logs. Activation of `ENABLE_PASSAGE_MAPPING` is a separate later decision with its own evidence.
...
5. **Small follow-ups from today, no spend, when next in those files:** `UnknownElementCard` renders `uncertainty` without the Fix 1 gate — route it through `elementCaveatNote` · the 404 console error on `/r/` pages (unidentified resource) · header "N of M elements covered" vs GAPS "M of M have evidence" (counter semantics) · the outreach machine's rule: **verify the physical send route when a roster row is built, and write the note for that channel** — apply to round two (Macfarlane, Gid M-K, Ridley) before any note is drafted.
