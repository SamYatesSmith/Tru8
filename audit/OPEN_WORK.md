...
> **2026-09-08 — Owner revision viewer implemented and retry header fixed.** Completed reports now expose retained before/after strengthening snapshots, source ledgers and element/relationship comparisons without replacing the current report. Fixed the API helper dropping Idempotency-Key, restoring stable-key transmission on research retries. TypeScript, 183 web tests and 10 PostgreSQL research/revision checks passed. This is local implementation, not live browser/end-to-end acceptance or deployment. Passage/structured-extraction candidates remain OFF; their broader semantic acceptance is still open. Next: public/export revision identity and live strengthening/revision acceptance, alongside the remaining original quality plan. Details: [revision viewer checkpoint](2026-09-08_revision_viewer.md).
