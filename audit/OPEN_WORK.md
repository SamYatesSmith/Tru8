...
> **2026-09-08 — Applicability candidate evaluated; rollout still blocked.** General element/time instructions are implemented behind the existing OFF passage switch. A 20-arm fixed-source comparison removes the fictional page-clock error and the observed cross-element challenge in this sample, while preserving positive controls, but real-page historical applicability still fails. Added hash-bound offline semantic regression checks: the combined candidate fails 3 of 11 targeted expectations, so this is not an accepted fix. 44 backend checks and the exact default replay baseline passed. Next: explicit, validated fact-applicability admission across initial mapping and passage completion, then varied held-out controls. No maintained runtime facts, model change or deployment. Details: [applicability evaluation](2026-09-08_applicability_evaluation.md).
