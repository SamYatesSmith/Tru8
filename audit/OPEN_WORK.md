...
1. **Measure the decomposition build — three paid runs, ask before EACH** (`tmp/astra-regrade.py --arm default` + `backend/scripts/review_labels.py`, ~£0.50 + ~2p per run). One run is noise; the claim is about the blind label rate (was 86.1% → 81.6%; the target is Astra's 95%). Then the **corpus `--record` ONCE** (the prompt change re-keys every cassette; 018F is already owed) and fix the README header. ⚠️ Do not run the bench before the re-record and read drift as regression.
...
**Bench (canonical: `backend/tests/replay_corpus/README.md` header):** last known `117 ok / 4 warn / 10 fail` + 3 drift, **now stale — every cassette re-keyed by `2957133`**; re-record after item 1's measurement. Web: 202/202, typecheck clean at sign-off.
