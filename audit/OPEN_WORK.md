...
> **2026-09-09 (late) — FINAL REGRADE on the finished default path: about 8/10 by Claude's grading on Astra's own 14 inputs** (14/14 complete, p90 63 s, ~£0.45): every input keeps its structure, both headline failures gone, remaining misses = SELECT composite endpoint, one residual premise element on creatine and Venus, EV decomposition variance. **Astra's 8/10 GATE is still unmet** — human review of ≥200 relationships, usability study, stall trace are the founder's. Record: [regrade](2026-09-09_astra_regrade.md) final section.
>
> **2026-09-09 (evening) — ASTRA REGRADE: default arm ≈ 7/10, NOT 8; candidate NOT ready.** Astra's 14 inputs re-run in both arms (28 checks, ~£1.00). Fixed since Astra: Bank Rate 3.75% supported, sqlite.org challenges "cannot occur", 4.25% challenged, abstention, 14/14 completed, strengthening coherent. **New, worse than anything Astra saw: 3 of 14 inputs had their contested conjunct silently dropped at EXTRACTION** (JWST's false orbit, Sweden's "caused", SELECT's "everyone") — fixed by `restore_single_thesis`, re-verification owed. ✅ **Corpus re-recorded 2026-09-09 (full `--record` + `--record-missing`, ~£1.20): pass state now `140/1/11/3 + 2 drift`** — 8/10 claims replay with zero misses; 82CF and 5647 (the historical flaky pair) are nondeterministic ACROSS PROCESSES at extraction (identical cassette → different claim text; root cause not isolated, timeboxed). 018F recital re-pinned 3/3 → 2/1 (the new decomposition rule drops its prerequisite element; the trap still fires). ⚠️ `--update-golden` dropped every curated invariant — restored by script; never commit its raw output. README header is canonical. ✅ **Extraction fix re-verified live on #2/#4/#6: JWST's false orbit reads `disputed`, SELECT's population reads `disputed`, Sweden separates no-lockdown / causation / ranking.** ✅ **Absence-of-evidence gate shipped (default-on)** — creatine-class "not enough evidence" challenges become context; live re-verification owed. Candidate breaks Bank Rate (true value → `context_only` via the fact-anchor gate) and adds wrong-direction passage supports on SQLite. Record: [regrade](2026-09-09_astra_regrade.md).
>
...
> **2026-09-10 (morning) — PHONE LAYOUT PASS on the record surfaces.** The founder's phone screenshot showed an element NOTE ending in "…": Fix 1 clamped it to two lines with the full sentence in a hover `title`, and a phone has no hover, so the sentence was simply cut (verbatim-or-nothing breached on the device most likely to hit the limit). Every lens of `/r/fa08cff7` was then sliced at iPhone-15 width (`verify-page.mjs --slices`, new) and read frame by frame. **Six faults fixed, all shared components so the dashboard gets them too:** the NOTE clamp removed (test now pins its absence) · the element roster's count + badge group was `shrink-0`, squeezing the description into a third of the row — below `sm` it now wraps beneath · the GAPS lens's known-element row truncated the description to four words — same wrap · the view selector was a fixed 3-column grid, so the public record's 4 tabs left MAP alone in a fourth cell — column count now follows the tab count (4 → 2×2) · the meta strip's four stats sat three-plus-one — 2×2 below `md` · two `<button>` links in the digest wrapped with their second line centred (UA default) — `text-left`. Verified locally against the prod API at phone width before/after: `design/preview/mobile-2026-09-10/` (gitignored). No horizontal overflow on any lens, the verify page, or the reading panel; the Codex-written notices (temporal details, passage review, revision history, record identity) are plain block text and lay out fine. ⚠️ Local `/r/` renders can serve a STALE `.next` fetch cache (`revalidate: 60` persists on disk across dev runs) — the first local render showed a 2026-09-04 response with no `reportIdentity`; don't read that as an API fault. **Second pass, same morning — the Evidence lens's classification guide** (`EvidenceHeatmap.tsx`, founder: "could be arranged in a more appealing and orderly manner", both widths): the tier line was stated twice in two wordings; "·" separators dangled at line ends on a phone; tier rows had a colour swatch and type rows did not, so labels never aligned; wrapped descriptions fell under their labels; and the "(rows)/(columns)" headings were wrong on a phone, where the grid is transposed. Now one legend (stacked list on a phone, one row from `sm`, swatch as the marker, no separators) and one guide rendered as an aligned definition grid (marker · label · description columns, axis-free headings), toggled by "What the tiers and types mean". Verified desktop + phone locally. **Third pass — the Evidence lens FILTER BAR** (`FilterPills.tsx`, founder: "not particularly clear", all sizes): twelve identical pills in one wrapping row, the only grouping two hairlines hidden below `lg`, a Diagnostic toggle orphaned beneath, no feedback on what a filter did. Now three labelled segmented strips that wrap as units (TIER / TYPE / LEANS — caption cell + touching pills, shared borders like the view selector), the diagnostic switch on the same row right-aligned and labelled "Diagnostic weighting" (a view mode, not a filter; its own line on a phone), and one status line whenever anything is active ("Showing 3 of 13 sources · Tier Primary · Clear all") which replaces the disposition-only "Showing…" panel. `.filter-pill` CSS deleted. Verified at 1280 + iPhone 15 with a filter active. **Second cut the same hour — founder: the three free-floating strips "still do not look co-ordinated"; "Diagnostic weighting is a problem, I don't understand its inclusion".** Now ONE full-width bordered panel, three rows of the same shape (fixed caption column + pills, rules between rows, shared left/right edge with the heatmap and ledger), and a fourth SHOWING row when anything is active. **The diagnostic switch is out of the filters** — it is a way of reading the ledger (ACH "diagnostic value": a source that supports one element while challenging another is marked with an orange left rule; context-only and same-direction-everywhere sources fade) — so it now sits in the ledger header beside Sort as "Highlight decisive sources", shown only when the check has any challenges. The ledger heading counts only when nothing is filtered (the SHOWING row already says "3 of 13"). **Third cut — founder: "garbled and jumbled … we need a fresh approach"; the diagnostic view "should stay, but it needs to be oriented".** The panel had rows hugging the left with dead space to the right and an orphan ACADEMIC cell on a phone; the diagnostic switch was small grey text in the ledger header, and only rendered when the check had challenges — hence "where is it?". Fresh approach = ONE control shape for everything under the heatmap, `SegmentedRow.tsx`: caption cell + EQUAL cells filling the full width (the view selector's idiom, which the founder already reads well). Filters = three such rows (TYPE wraps into two equal rows of three on a phone, six across from `md`); a one-line status beneath ("Select any cells to narrow the ledger — 13 sources shown" / "Showing 3 of 13 sources · Tier Primary · Clear all"). The ledger's own controls take the same shape: `SORT [Date|Source|Element]` and `VIEW [All sources | ◉ Highlight decisive]` side by side from `md`, with a one-line definition of "decisive" beneath. The native `<select>` sort is gone (`SortControl.tsx` now only exports the `SortField` type). Verified 1280 + iPhone 15 with a filter active and with the highlight on.
>
...
> **2026-09-15 in-day log (agent, live):** 09:35 — Send Board now runs LOCALLY: `python audit/recipients/send_board/serve.py` → http://127.0.0.1:8765 (gitignored dir; `board.html` = Monday's artifact page + a local adapter that writes every tick to `state.json` beside it, so the agent reads ticks off disk; the claude.ai artifact still works unchanged when opened there). State seeded from the artifact db: wave 0 UNSENT as of Monday close (only TTE's version chosen). Wave 1: 25 topics verified, 0 records run.

> **2026-09-21 in-day log (agent, live):** Founder review of the outreach approach ("a man on a solitary chair in a field"): the plan never relied on a following, but 2 sends in 10 days showed the per-send decision cost was the real block. Founder direction: **automate the lot** (agent sends, founder approves a day's batch) — starting with a **10-email batch**: verified addresses, records ≥ B+, notes, overview sheet, then send. Address re-verification 10/10 on-page + MX; topics re-verified against each writer's LATEST piece (`audit/recipients/wave1/2026-09-21_topics.md`, 14 verified / 1 stale); **Alison Moore HELD** (both fresh HSJ pieces paywalled with the trust/name invisible — claim unsearchable); the HSJ £800k piece is bylined **Zoe Tidman**, not McLellan (editor) — swapped. Go given for 12 records (10 + 2 spares). **First record hit `Insufficient balance for full tier`: the agent rail bills a prepaid balance the founder must top up to pay himself, and `_require_console_submission` (`checks.py:112`) 403s API-key submission on the subscription's `/checks` endpoints by design.** Founder: *"WHY can this not just be run through my subscription?!"* — right. **Built the permanent fix (founder's call, same hour): an `ADMIN_EMAILS` account submitting with its OWN API key passes the wall and is billed to its subscription** (`initiated_via="api_key"`, an existing vocabulary value); needs the RESOLVED auth method to be `api_key` — a bare header never qualifies; empty allowlist walls everyone; all three call sites (stream, sync `/run`, research ops) pass `current_user`. Helper `usage_ledger.is_admin_email`. Guards: `tests/unit/api/test_checks_path_separation.py::TestAdminApiKeyExemption` (6 new; 20/20 file, 26/26 `test_checks_api`, 20/20 users endpoint). Full suite NOT run — Docker was down and the suite hangs on Redis/Postgres, not logic. After deploy the records go by REST `POST /api/v1/checks/run` (`{"input_type":"text","content":…}`, `X-API-Key`), 1 subscription credit each, no agent balance. ⚠️ Requires the founder's API-key account to be on Railway `ADMIN_EMAILS`. **✅ LIVE-PROVED 15:36 BST: record `8d66d41a` created on the founder's account via `POST /checks/run` with the API key, 1 subscription credit, `creditsUsed: 1`, no agent balance touched.** ⚠️ The sync endpoint's HTTP response is cut at exactly 60 s by something upstream of the app (the pipeline carried on and completed at 81 s) — submit, then pick the new id off `GET /checks/?limit=1` and poll `GET /checks/{id}`; never resubmit on a dropped response.
>
> **2026-09-21 batch result (afternoon): 12 records run · 4 SEND (B+) · 8 HELD — `audit/recipients/wave1/SEND_SHEET_2026-09-22.md`.** SEND: Tidman (HSJ) · Kennedy (Energy Flux) · Legum (Popular Information) · Katz (The Racket) — every note pressure-passed on the page + every lens + payload, then fact-checked by a fresh agent (2 of 4 held once and fixed: Tidman named a source the page doesn't; Katz's "that sentence" where the record merges two). HELD: Turns C · Morris C+ · Buck B− · Anapol B · O'Doherty B · Taylor B · Panthagani B · Burke-Kennedy B, notes drafted in each `<surname>.md` §4 for override. **Founder still owes the registered-address footer line before any email goes.** Two defects the passes exposed were FIXED the same hour (local commits, pushed with this entry): **"Note · None." printed literally** when the mapper writes the sentinel as a sentence (`_clean_uncertainty` + `elementCaveatNote`, `728fe1c`) and **the GAPS lens printed raw `uncertainty` with no verdict-language gate** — "The evidence directly confirms…" under an UNRESOLVED badge on a public page (`UnknownElementCard` now uses the Fix 1 gate, `4816a04`).
>
> **Product observations from the 12 (NOT built — procedure §10; each seen on ≥2 records, ranked by how many holds it caused):**
> 1. **Notables "Most relevant supporting source" is chosen by raw retrieval `relevanceScore`, not by mapping or tier** — it headlined a stillbirth charity's post above the inquiry's own text (Tidman), a bare `youtube.com` link (Anapol), an undated PA copy (O'Doherty), the WRONG paper for a writer whose piece is about studies cited for results they don't contain (Panthagani), and the recipient's own column (Burke-Kennedy). One derivation change; it caused or worsened 4 of 8 holds. Rank by: mapped `supports` on the most elements → tier → topical score; exclude the claimant/recipient's own piece.
> 2. **Recital + interested-party gates mis-fire on attribution claims ("X has said Y" where X is the subject):** the Thirlwall report page filed context (Tidman), the Central Bank's own Q3 bulletin demoted on `name_in_domain` (Taylor), Cook's methodology PDF demoted on a claim ABOUT Cook (Morris), Guardian/AP demoted on "announced" while the Conversation piece itself counted as support (Anapol). Plus a loose-token match: `politicalwire.com` on "political", `inquirytracker.uk` on "inquiry". The subject of an attribution claim is not an interested party in the gate's sense; a receipt for the claimant's own organ needs the claimant, not the subject.
> 3. **Temporal blind spots:** the gate cannot fire when the period lives in a SIBLING element (Morris: date only in element 01, so April-wave items challenged 02); a bare "Q3 2026" is read as `Period(2026, None)` and counted as a different period from 2026-09 (Taylor); a partial-period snapshot (GAO March 2018: "37 models") filed as a challenge to a 2010–2020 total (Buck); a 2021-vintage "416 ppm" recital disputes "above 420 ppm now" (Turns).
> 4. **Planner windows every query when the claim says "now"** (`pw` on all five current-level queries) and the Build A unwindowed twin is exempt on `pw` — so the claim's own words never ran unwindowed and NOAA/Scripps never entered an 8-source pool (Turns). The twin's pd/pw exemption was designed for genuinely time-boxed claims; "now" is the opposite case.
> 5. Surface: a stored Eurostat title carries raw `<span class="mw-page-title-main">` HTML through the ledger, TIMELINE and PDF (Burke-Kennedy — `cleanTitle` only trims suffixes) · VIDEO lens off-topic (an Elon Musk solar clip on an Irish spending claim; five unrelated videos on Panthagani) — the recommender has no relevance floor · "Publication date: 1 day ago" / "6 days ago" stored and printed verbatim (Anapol, Taylor, O'Doherty) · two hosts of one article un-deduped (bbc.com/bbc.co.uk on Katz; PoliticsPA on Morris; JHM on Panthagani — the 16 Sep invariant-1 finding again) · the same Galway release counted twice under two URLs (O'Doherty) · the disputed/contextual card renders NO explanation when the mapper's sentence contains "confirm" (Turns, Taylor — the Fix 1 gate hiding a negated limit).
> 6. Two Conversation rows are EDITORS: a record about a piece they commissioned is a weaker unit than a bylined writer's own claim; the Anapol claim chosen was uncontested reportage (the tool can only say "the papers said so") — prefer the piece's substantive claims.

> **2026-09-16 in-day log (agent, live):** Send Board up locally 09:xx. Founder replying to TTE's 16 Sep newsletter email ("Introducing Trust the Evidence Research Reports") — subject kept as theirs; paid-only reply gate is a real risk (Substack default), so it is attempt 1 with the DM as the send if nothing in 48 h. Note rewritten as an email (no 150 cap): responsive to their reports post (readers as the peer review), one sentence of the founder in his voice, no ask. **TTE re-run `a57b5494` (15p) — B+, CHOSEN over the 2 Sep record:** their piece now filed as the CHALLENGE on the causal part, recitals of NHS England's number filed context, Engelen gone; wart = the challenged state rests on their piece alone (thin badge renders). Design observation: no challenge-side floor — a lone commentary challenge with zero supports reads `disputed` while a lone commentary support reads `unresolved`; founder call, no build. Wave-1 row #24 (Heneghan, UKHSA forecast) is the SAME recipient as wave 0 — hold it. Record: `audit/recipients/wave0/tte.md` §5. Spend today: 15p. **15:12 — the email reply BOUNCED (Substack: paid subscribers only; the gate flagged at 09:xx was real). Un-logged; TTE goes by Substack DM with the 169-word DM variant. Durable: a Substack newsletter reply from a free subscriber is not a route — check the paid-only gate BEFORE building an email for it, or DM.** **16:24 BST — VIGLIONE SENT via the Carbon Brief form (record `441144ac` kept, 159-word note, fact-checked PASS) — the campaign's first send.** **16:43 BST — SEYMOUR SENT via Substack DM (record `580fd5b4`, 150 words, fact-checked PASS).** **Tapper: founder chose his 13 Sep post over the 26 July Laffer thread — it is Fraser Nelson's Substack piece REPOSTED (say 'the piece you reposted'). New record `1ca0070f` (15p, B+): £72m + two £36m supported with a received-vs-pledged note; the 'more than any party in the history of British democracy' superlative UNRESOLVED (record single donation ≠ party totals) — 03 renders no Note explaining why (reason is click-through only). 141-word comment on the board, fact-check running. Spend today: 30p.** **DEFECT found by the fact-check on `1ca0070f` (engineering, not tonight): cross-host dedup miss — `bbc.com/news/articles/c3v4zvyde15o` and `bbc.co.uk/news/articles/c3v4zvyde15o` are ONE article and both sit in the pool, one filed `supports` and one `context` on the same element (invariant 1: URLs tracked globally). Fix = normalise host aliases (bbc.co.uk↔bbc.com at least; check the path-id form) in the global URL tracker before fetch. Also: element with `uncertainty: null` and `support_floor` renders no Note — a reader cannot see WHY it is unresolved; consider rendering the floor reason as the Note.**

**Read in this order:** this section → `audit/2026-09-11_outreach_operating_procedure.md` §11 (the Monday-morning script) → `audit/recipients/wave0/SEND_SHEET_2026-09-11.md` (what the founder was sending Friday) → `audit/recipients/wave1/2026-09-14_topics.md` (Monday's 25). Everything below this section is history and evidence.

**What Friday 2026-09-11 was, in one paragraph.** The founder resolved the bespoke-vs-volume contradiction: **tailored at volume — automate the record and the draft, never the message; the founder reads every record before it goes** (`audit/2026-09-11_outreach_operating_procedure.md` is the SOT for HOW sends happen; `OUTREACH.md` stays the SOT for WHAT has gone and the verdict table). Budget: £750–£1,000 available, waves 0–1 under £20; no sequencer, database or ads before the wave-1 verdict (Mon 21 Sep, 50 sends + one week). Queue built: `backend/scripts/outreach_queue.py` → `audit/recipients/queue.csv` (gitignored; 667 rows; wave 1 = 30 journalists + 20 newsletter writers, ≤3 per outlet; approach day/time by route; state columns per recipient). **Wave 0 delivered** (five notes, each pressure-passed on page + every lens + PDF and fact-checked sentence-by-sentence by a second agent; two rewritten on real errors) — **the founder was reading and sending by hand as the session closed; the agent does NOT know what went.** Monday's 25 topics verified 25/25 from the recipients' own pieces (three are rebuttals; one is a 2022 reprint to swap). Engineering: **idempotency rebuilt** (a fixed clock bucket → no time term + API-key salt + server-side sliding window + payer check; live-verified: a resend 6 min later returned the same check at 0p) after a **self-inflicted 15-minute agent-path outage** (an ORM write reorder → FK violation; fixed, test-pinned, no money lost); **agent-rail refund parity built** (agent runs register with the shutdown guard; both sweeps refund `credit_balance_pence`; the stale-check sweep runs every 5 min) — deployed, unexercised live. **Variance traced to origin:** on a byte-identical cached pool the classifier flips 2/16 tiers, the mapper emits 13 vs 11 refs, decomposition changes the element set, and coverage recovery amplifies — design + a ~£1 measurement written, not run (`2026-09-11_determinism_design.md`). Spend Friday: six wave-0 re-runs (90p, one duplicate), one stranded probe (15p, unrefunded at the founder's choice), one probe pair (15p). Everything is on `main` and deployed.

**▶▶ MONDAY, in order — the agent PROMPTS the founder for each; do not wait to be asked:**
1. **First ten minutes — five questions, answers logged before anything else:** (a) Which of the five wave-0 notes went, by which route, at what time? → the log table at the foot of `SEND_SHEET_2026-09-11.md` + the FIRST TOUCH line in `OUTREACH.md`; (b) any replies over the weekend (mailbox, Substack, Bluesky, Carbon Brief form, X, blog comments)? → procedure §4H, comp on a bite; (c) Substack subscriptions (Trust the Evidence, Seymour) and the Bluesky DM setting done? (d) `audit/recipients/review_flagged.csv` marked up (7 held contested-voice writers + 55 flagged rows)? → merge into `queue.csv` (`founder_row_ok`, or `status=rejected`); (e) **go for Monday's 25 records — 25 × 15p agent price = £3.75 nominal, ~30p real. ASK; never assume.** Also: Railway watch paths set to `backend/**`? (five redeploys Friday, three docs-only; each is a stranding window for in-flight checks.)
2. **Run the 25 records ONE AT A TIME** — never five in parallel through the hosted MCP tool (Friday: one 502, one dropped stream, one timeout, one duplicate charge). The MCP tool sequentially, or REST `POST /api/v1/agent/check` with the key from `~/.claude.json` held in a header file (`curl -H @file`; Python `urllib` gets a Cloudflare 403). `max_age_hours=0`, `max_tier=full`, `compact=true`. **Write the full check id into `queue.csv` the moment each completes** (`check_id`, `record_url`, `charged_pence`, `status=record_run`). Three rows are REBUTTALS (Ian Dunt, Judd Legum, G. Elliott Morris): a record that misses the rebuttal → `held: rebuttal_missed`. Stuart Buck: swap the 2022 reprint for a current piece or hold.
3. **Pressure passes + notes + fact-check passes as subagents** — the Friday pattern that worked: one agent per record (procedure §4C + §4D → `audit/recipients/wave1/<rid>.md`), then a FRESH agent per note (§4E → `<rid>.factcheck.md`), fixes applied by the lead with a word re-count. **Expect about half to be held** (Friday: 3 of 6 fresh records graded C). Assemble `audit/recipients/wave1/SEND_SHEET_2026-09-15.md` in the wave-0 shape (record · grade · route · before-you-send · note · log table).
4. **Timing:** Monday's rows carry `approach_day=2026-09-14` but their records cannot be ready before the 09:30–11:00 email slot — **that batch sends TUESDAY at its slots.** Rebuild the remaining queued rows with `python backend/scripts/outreach_queue.py --start 2026-09-16` so the rhythm is: verify topics day N (free) → run records + passes day N+1 morning (after the go) → founder reads day N+1 → sends day N+2 at the slots.
5. **Founder reads** (~3 min per record) → edits applied → any edit re-runs that note's fact-check → founder sends by the route on the row, ≤2 per outlet per day, never both Carbon Brief notes to the form on one day → agent logs `sent_on`, `sent_by_route`.
6. **Prepare Tuesday's topics** with the topic-verification agent (free; fetch the latest piece, one checkable claim in the author's words, role/route re-verified, holds with reasons).
7. **Weekly numbers line in `OUTREACH.md` on Mon 21 Sep:** sent · replied · said-useful · ran-own-check · signed-up (`python -m scripts.signup_sources`) · opt-outs · bounces → the verdict table.

**Engineering owed — do NOT start Monday unless the outreach day is quiet or the founder asks:** PyPI `tru8-mcp` 1.0.4 (stdio users still send the bucketed key; add `remotes[]` to `server.json` in the same release; founder's release) · the ~£1 frozen-pool variance measurement (ask first; Friday's cached pools have EXPIRED — run any claim twice inside 24 h for a fresh replay pair) · V1–V3 determinism builds on the measurement's word · V6 author-supplied sources with receipts (the commercial assessment's product change; founder decision) · run `refund_stranded_agent_transaction` once against tx `a3971739` to exercise the new refund path live (a 15p prod write; the founder said forget the 15p) · factcheck-outlet lane probe (pence) · the idempotency "terminal" set includes `unsettled` — revisit once strandings refund promptly.

**Durable rules from Friday (also in memory):** a fresh re-run is not automatically the better artefact — pressure-pass, then choose old or new · the second, fresh-context fact-check agent caught real errors in 2 of 5 notes — never skip it · parallel long calls through the hosted MCP transport fail — sequential or REST · an ORM write reorder is a schema-level change: run it against Postgres before deploying · every push redeploys Railway and every deploy is a stranding window · an idempotency key must carry no time term and must carry the caller · the 24 h evidence cache makes any re-run a free instrument for classify/map churn.

---
## Previous START HERE — Friday 2026-09-11 (written 2026-09-10 late morning, sign-off)

> **✅ 2026-09-11 morning — OUTREACH RECONCILED, founder decisions taken.** The register's "no automation, no templates, no volume" and the volume plan's sequencer motion were contradicting each other; the founder's answer is **tailored at volume — automate the record and the draft, never the message; every record read before it goes.** Procedure (guidelines, roles, states, standards §4A–4H, daily 6-hour session, schedule Fri 11 → Fri 18, queue schema, cost): **`audit/2026-09-11_outreach_operating_procedure.md`**. Decisions recorded in `2026-09-10_outreach_volume_plan.md` §9 (segments = journalists + newsletter writers; list = the 667 rows; hand-sent, no sequencer; dev launch later; waves 0–1 under £20 of a £750–£1,000 ceiling — usability study + one niche-newsletter slot only after the verdict; never ads). New tool `backend/scripts/outreach_queue.py` → `audit/recipients/queue.csv` (gitignored): priority, wave (1 = 30 journalists + 20 writers, 2 = rest of those segments, 3 = other segments held for the verdict), approach day/time by route and country, channel, and every per-recipient state column. **Verdict Mon 21 Sep at 50 sends + one week.** Wave 0 today: the five bespoke records re-run by the API route (5 × 15p agent price, ask first), pressure pass, note fact-check, founder reads and sends by hand. The bar question ("does 89.4% blind / 8/10 structure clear the founder's 8/10 gate?") is still the founder's to answer before wave 0 goes.
>
> **2026-09-11 ~10:00 — GATE: founder said "FOR NOW — yes"; wave 0 re-runs GO (75p approved). Ran via the hosted MCP tool, five in parallel: SIX checks were charged (90p), not five.** NHS `00a59d64` (3/3 supported — causation now supported on two resultsense.com reports; was UNRESOLVED on `1bbf008a`), Scotland `8c74787e` (3/3 supported; futureeconomy.scot in pool; recital gate scoped 2), heatwave `a093c7d2` + `bebfa026` (both with carbonbrief.org; the second has e3 UNRESOLVED), wildfire `580fd5b4` + `12f607d1` (both 2/2 disputed; **carbonbrief.org in neither** — Viglione's own factcheck missed again, the Phase D structural finding). The Seymour text never ran (DNS `ENOTFOUND`, then "unable to connect"); he shares the wildfire record as in August. ⚠️ **TWO PRODUCT OBSERVATIONS, not built today:** (1) five parallel full checks through `POST /mcp` produced one Cloudflare 502 (origin), one dropped stream, one timeout — the hosted transport cannot carry parallel 60–90 s tool calls; run them sequentially or via REST `/agent/check`. (2) **The `Idempotency-Key` replay did NOT fire on the resends** — the heatwave resend came 6 min after the first (inside the 10-min window) and still created a NEW check and a NEW 15p charge; the 2026-09-02 fix (`tru8_mcp/tools.py` key = endpoint + payload + window; `agent.py` returns the original when `charge()` finds an owning transaction) needs a live probe: either the key differs between calls (stateless_http → a fresh server process per request computes a different window bucket?) or the first check had not yet reached `charge()` when the resend arrived. Pressure passes running as four subagents → `audit/recipients/wave0/*.md`.
>
> **2026-09-11 ~10:40 — WAVE 0 PRESSURE PASSES DONE (four agents, page + every lens + PDF). Result: 2 of 3 fresh records are WORSE than their 2 September predecessors for their recipients and are HELD; the send set is mostly the OLD records.** TTE → OLD `1bbf008a` (B; NEW `00a59d64` C: causation SUPPORTED on two resultsense.com pages, one of them the site's own TAG INDEX, both reciting NHS England; the 29% supported by NHS England's own FOI reply on whatdotheyknow.com filed PRIMARY/OFFICIAL — the claimant's answer counted as support for the figure it explains; no NOTE renders). Tapper → OLD `4be28cd1` (B−; NEW `8c74787e` C: decomposition MERGED the contested £53m conjunct into two near-duplicate elements and added a trivially-true "Scotland implemented a 48p rate" — supported ×3, zero challenges, "Most relevant supporting source: breitbart.com" dated today by `page_metadata` for a 31 Jul article; the recital gate fired on futureeconomy.scot's REBUTTAL because it quotes what it rebuts). McSweeney → NEW `bebfa026` (B; e3 UNRESOLVED shows his structure unprompted; seam: rocketnews.com, an AI aggregator, rewrote his piece FIVE times and each copy is a separate REPORTING/NEWS challenge row — no same-article identity gate; `isFactcheck: true` but the badge never renders because the rating component needs publisher+rating). Viglione → OLD `441144ac` (B; carbonbrief.org in NEITHER new pool). Seymour → NEW `580fd5b4` (B−). **Mechanism finding: the two wildfire "runs" were ONE retrieval — the second replayed the first's pool from the 24 h evidence cache (identical 16 titles, 10 s vs 25 s) and classify + map DISAGREED with themselves on four items** (Senedd primary↔commentary, ISRM commentary↔reporting, Copernicus mapped↔unmapped, thin flag present↔absent) — so part of run-to-run variance is classify/map, measurable for free by replaying one cached pool N times. ▶ **Product items named by the passes (none built today):** same-article identity gate (aggregator copies) · a claimant's FOI reply / tag pages as independent support · non-causal element lost the claim's direction ("difference is substantial"; direction repair covers causal only) · recital + echo receipts and thin-sourcing expansions render PDF-only · factcheck badge never renders on page-classified factchecks · verdict language passed the NOTE gate on `12f607d1` ("directly contradicting the claim") · counters disagree (reviewed 17 / organised 22; "9 bear directly" matches nothing) · `page_metadata` dates (Breitbart today; GB News 8 May) · PDF prints each NOTE twice · decomposition duplicating the contested conjunct on a contested-conjunct claim (`restore_single_thesis` protects extraction, not decomposition). Full detail: `audit/recipients/wave0/{tte,tapper,mcsweeney,wildfire}.md` §6.
>
> **2026-09-11 ~11:45 — WAVE 0 SEND SHEET DELIVERED (`audit/recipients/wave0/SEND_SHEET_2026-09-11.md`): five notes, every one pressure-passed and fact-checked by a second agent; two rewritten on fails (McSweeney misquoted the Telegraph headline; Tapper's DM attributed the £53m framing to him). Monday's 25 topics verified 25/25 (`audit/recipients/wave1/2026-09-14_topics.md`). ▶ FOUNDER: read + send; log what went.**
>
> **2026-09-11 ~12:00 — TWO ROOT CAUSES FOUND, DESIGN WRITTEN: `audit/2026-09-11_determinism_design.md` (read this; it cites the three investigations).** (1) **Idempotency:** the MCP client key is `floor(epoch/600)` — a fixed clock bucket, so a retry after gap g re-charges with probability g/600 (57% this morning); PLUS the key carries no caller identity and the server looks it up by key alone → **two hosted users sending the same claim in one bucket: the second gets the first's check uncharged** (billing + privacy hole, live on `/mcp`); PLUS the smart endpoint 409s a retry when `max_age_hours` is default. Fix ≈ 1 h, no spend, designed with tests (`2026-09-11_idempotency_root_cause.md`). (2) **Variance:** on a byte-identical cached pool the classifier flipped 2/16 tiers (both `llm`, temp 0.1, no seed), the mapper emitted 13 vs 11 refs, and decomposition (temp 0.2) changed the element set on the heatwave pair — which cascaded through coverage recovery into a different state. Prod confirmed `MAPPING_THINKING_BUDGET=0`. Google client never sends `seed`. Design: V1 seed+temp 0 on classify/decompose/map · V2 per-source classification memo · V3 per-claim decomposition memo · V4 explicit mapping coverage (measure first) · V5 recovery receipts on the page · V6 author-supplied sources + factcheck-outlet lane. **Measurement owed before building (~£1, ask): frozen-pool repeats per stage, both arms; the wildfire pool key lives until ~09:47 UTC 2026-09-12.** ▶ Founder decisions: go for the idempotency build · go for the £1 measurement · V6 as its own decision.
>
> **✅ 2026-09-11 ~12:40 — IDEMPOTENCY REBUILT (founder: "Go… build it now").** Client key = endpoint + payload + sha256(api_key)[:16], no time term (`tru8_mcp/tools.py`); server-side sliding window `AGENT_IDEMPOTENCY_TTL_S=600` from the first transaction's `created_at` (`agent_auth.idempotency_disposition`: replay / expired / other_payer / different_request); payer check (another caller's key → 409, never a replay); expired keys retired as `key#txid`; lost insert race → rollback (undoes the debit) and return the winner; smart endpoint decides the key BEFORE tier resolution (`agent._replay_for_key`); tx linked to its check in the same commit that exposes it; x402 gained the replay guard. Tests: 3 files, +17 (37 in the three idempotency files; agent + MCP suites 203 + 38 green). ⏳ **Owed:** PyPI `tru8-mcp` 1.0.4 (stdio users still send the bucketed key until they upgrade; the hosted `/mcp` route is fixed on deploy) · a live probe after deploy: two identical `tru8_check` calls 6 min apart → second returns `X-Tru8-Idempotent-Replay: 1`, `chargedPence 0` (~15p, ask).
>
> **⚠️ 2026-09-11 12:24–12:40 UTC — SELF-INFLICTED OUTAGE ON THE AGENT RUN PATH, ~15 min.** The `19fe47e` "link tx to check in the same commit" reorder set `tx.check_id` before the check row was flushed; SQLAlchemy issued the tx UPDATE before the check INSERT and Postgres refused the FK (`agent_transaction_check_id_fkey`, request `465c4d2b`) — **every `/agent/check|quick|full` call 500ed from the deploy until `81f2b29`** (flush the INSERT first; pinned by `TestCheckIsInsertedBeforeTheTransactionLinksToIt`, `7d342f7`). Found by the first probe call, not by tests: the unit suite mocks the session, so ordering against a real FK was never exercised. No charge and no check row survived (the debit lived in the same session and rolled back; verified in the DB: 6 tx rows today, all pre-noon). Durable: **a reorder of ORM writes is a schema-level change — run it against Postgres (integration) before deploying, not only against mocks.**
>
> **✅ 2026-09-11 12:53 UTC — LIVE IDEMPOTENCY PROBE PASSED on `81f2b29`.** Call 1 (12:45:56, "UK CPI 3.8% August 2026", full, `max_age_hours=0`) → check `ee664153`, tx `1f883dcb`, 15p. Identical call at 12:52 → **same check id, `chargedPence 0`, no second transaction row** (DB: one tx after 12:30, one check). The key was the same on both (`mcp-fe07f5fa…`) — no clock bucket. ⚠️ **Found on the way — a THIRD money leak, pre-existing:** the first probe attempt (12:27:02) hit the OLD container seconds before the deploy killed it: it charged 15p, created check `5525b573`, and died mid-pipeline. That check is still `processing`; the tx was marked `unsettled / stale_pending` at ~12:37 by `agent_maintenance.sweep_stale_pending_transactions` (which changes status only, refunds nothing); the shutdown guard `inflight.fail_and_refund_inflight` and the boot sweep `sweep_stale_checks` refund via the DASHBOARD usage ledger (`refund_usage`, a no-op for agent checks with `credits_used=0`) and never touch `credit_balance_pence` — **an agent check stranded by a deploy loses its money on both rails, and the row is stranded forever** (the boot sweep only ages rows past 420 s, and 5525b573 was 33 s old at boot; there is no periodic check sweep). Balance moved 102p → 72p for ONE completed check. Owed: (a) refund the founder's 15p + fail `5525b573` honestly (founder's word — it is a prod DB write); (b) build agent-rail parity: the shutdown guard and the stale sweep must look up `agent_transaction` by `check_id`, `refund_credits` the payer, set `tx.status='refunded'`, and run periodically (piggyback the 5-min stale-pending loop) — same class as the 2026-07-21 fix (`57e7dcde`), which only covered dashboard credits. Also: at 12:45:56 the new-key call correctly treated the 12:27 tx as `expired` (unsettled + >600 s) and charged afresh — right by the rule, but the rule's "terminal" set includes `unsettled`, i.e. a stranded run counts as finished. Reconsider once (b) refunds strandings promptly.
>
> **✅ 2026-09-11 ~13:30 UTC — AGENT-RAIL PARITY BUILT (founder: "forget the 15p for now… proceed as required").** `app/services/agent_refunds.py::refund_stranded_agent_transaction` (credit rail → `refund_credits` to the check's owner, `tx.status='refunded'`, `settlement_reason='stranded'`; Skyfire/x402 → `unsettled`; never raises, logs ERROR; idempotent on `refunded`/`failed`; an `unsettled/stale_pending` tx is STILL refunded); called by both `inflight.fail_and_refund_inflight` and `inflight.sweep_stale_checks` beside `refund_usage`; agent runs (sync, async background, x402) now `inflight_register`/`unregister`; `agent_maintenance._stale_pending_loop` also runs `sweep_stale_checks()` every 5 min (boot-only before — the 33 s-old stranding was invisible to it). Tests: +9 (`test_agent_refunds.py` 7, guard 1, sweep 1); 210 green across guard/sweep/agent/MCP. **Consequence for `5525b573` — CORRECTED after deploy:** the row was ALREADY `failed` (STALE message) by the boot sweep of the 13:55 BST docs-only redeploy (old code: `refund_usage` only), 29 min after it stranded — so the new agent-rail refund never saw it; tx `a3971739` stays `unsettled/stale_pending`, balance stays 72p. The new code is deployed (`c687230`, 14:06 BST) but **unexercised live**; unit tests only. To exercise it on the real row: run `refund_stranded_agent_transaction` once against `a3971739` (a 15p prod write — founder said forget the 15p; his call). ⚠️ **Every push redeploys Railway — five deploys today, three of them docs-only — and every deploy is a stranding window for in-flight checks.** Founder action: set Railway *watch paths* to `backend/**` (exclude `audit/**`, `.claude/**`, `web/**`) so documentation commits stop restarting the API. ⏳ Still owed: PyPI `tru8-mcp` 1.0.4 · the ~£1 variance measurement (ask) · V6 decision.

> **⚠️ ADDENDUM, written 2026-09-10 ~16:00 by the pipeline session — READ THIS FIRST, it supersedes items 1–3 below.**
> **The afternoon:** the decomposition build was measured 3× (targeted kind 18/27 → 0/45; blind 87.5%) · build 2 (`FINDING, NOT TOPIC` mapping rule + causal elements keep the claim's direction + fact-check snippet, `32d3a2f`) measured 3× (**89.4% blind, ≈94–95% adjudicated; Sweden misreads 5 → 0**) · a build-3 premise check found only ~0.8 points recoverable mechanically · **the founder DREW THE LINE: no build 3; label axis final for this push at 89–91% blind / ≈95% adjudicated; structure 8/10.** Corpus re-recorded twice (`170/5/8/5` + 82CF drift, README canonical). **Everything is PUSHED and LIVE — production `6ab374c`.**
> **Founder's rules from the afternoon:** *"I want an 8/10 output BEFORE any sends go out"* (the "stop pipeline work" line was Claude's, struck) · *"Everybody and anybody whom may show an interest. Any way to contact them is welcome. Collect all recipient information and then arrange a mass contact styled methodology."*
...
> **▶▶ TOMORROW, in order:** (1) **the five bespoke records must be re-run on the deployed build** — founder to choose: agent via API (5 × 15p) or founder from his console (5 plan checks); then the pressure pass (rendered page, every lens, PDF) and note rewrites; (2) founder's five decisions on the volume plan (segments · list route · sending tool + subdomain · developer launch in parallel · budget) — nothing in the kit is built until then; (3) founder skims `master.csv` review flags + the contested-voice rows the newsletter collector flagged (Goodwin, Berenson, Kirsch, Malone, Campbell, Fenton, Craig — his call); (4) second sweep for the blocked sources if wanted (founder's browser gets past the Cloudflare walls the fetchers could not). Sends: wave 0 = the five bespoke notes, by hand, on the founder's word.


**What happened on 2026-09-10, in one paragraph.** Two threads, both on `main`, all pushed and live. **(A) Record surfaces, founder-driven from phone screenshots — five UI commits** (`b8c611c` → `3db9dbc`): the element NOTE was cut to "…" on phones (a hover `title` is no fallback on a touch screen — clamp removed); six phone-layout faults across the roster, GAPS rows, view selector, meta strip and two link-buttons; the Evidence lens classification guide became one aligned definition grid; and the Evidence lens controls went through THREE cuts before the founder accepted the shape — **one control idiom for everything under the heatmap, `SegmentedRow.tsx`** (caption cell + equal cells filling the width, the view selector's shape): filters TIER/TYPE/LEANS, then `SORT [Date|Source|Element]` and `VIEW [All sources | ◉ Highlight decisive]` side by side. The founder was asked what "Diagnostic" is (ACH diagnostic value: a source that supports one element while challenging another; context-only fades) and ruled **it stays**, oriented and explained. `verify-page.mjs --slices` is the new phone-review tool. The record-page 404 console error is Google's favicon service — identified, cosmetic. **(B) In a parallel session, the decomposition-specificity build landed** (`2957133`, `audit/2026-09-10_decomposition_specificity.md`): prompt rule on both decomposers + the grounds builder, `unstated_quantity.py` backstop — **suite green, NOT measured, corpus re-record owed.**

**▶▶ WHAT HAPPENS NEXT (in order)**
1. **Measure the decomposition build — three paid runs, ask before EACH.** ✅ **RUN 1 DONE 2026-09-10 12:42 (~52p): 87.6% justified (120/137, primary reviewer), 17 rejections, both-reviewers-reject 4 (was 13) — and ZERO of the "unstated specificity" kind (was 18/27).** EV grounds questions came out comparative. Remaining kinds: reviewer direction error 2 · the claim's own absolutes 6 · passage-level mislabels 5 (rating-only fact-check windows; topic-without-finding) · arguable 4. New small fault: t06's causal element dropped the claim's direction ("lower"). ✅ **RUN 2 DONE 13:03 (~52p): 86.0% (92/107), 15 rejections, again ZERO unstated-specificity; no specificity word in any of 41 elements.** Remaining kinds hold: claim's own absolutes 5 · reviewer direction error 2 · passage-level mislabels 6 (Venus temperature-without-comparison ×3; AFP rating-only window on BOTH runs; a pre-launch article; a 20 g protocol) · intervention-as-behaviour shape once (prompt-only) · absence gate misses "inconclusive" once. ✅ **RUN 3 DONE 13:28 (~52p): 89.0% (105/118), 13 rejections, ZERO unstated-specificity.** **THREE-RUN READING (design doc §8): the targeted disease is 0/45 (was 18/27); blind rate 87.6/86.0/89.0 = mean 87.5%, adjudicated ≈93% — SHORT of the 95% bar by ~7.5 points, ~9 labels a run.** The 45 remaining, by kind: passage-level "topic without the finding" 17 (mapper; 2 of them the fact-check adapter's rating-only snippet, `factcheck_api.py:229`) · the claim's own absolutes 14 (not ours) · reviewer misreads 7, five of which sit on a causal element that DROPPED the claim's "lower" · decomposition shape 4 · arguable 3. **Next build the measurement names: (1) mapping rule "finding, not topic" + the adapter snippet fix; (2) causal elements keep the claim's outcome direction (prompt + mechanical check).** Both re-key mapping cassettes → re-record after. ✅ **BUILD 2 DONE 2026-09-10 afternoon (`audit/2026-09-10_finding_not_topic_direction.md`), founder said proceed:** `FINDING, NOT TOPIC` rule on both mapping prompts (every example a measured rejection) · fact-check fallback snippet now names the claim rated · `KEEP THE CLAIM'S DIRECTION` rule on both decomposition prompts · `app/utils/direction_fidelity.py` detector + `_repair_lost_direction` (one fail-safe 1→1 call, both decomposition paths, `metadata.direction_restored`, flag `ENABLE_DIRECTION_REPAIR`) · `_is_causal_link` gained `driver(s) of` (those elements had no causal tag at mapping either). 26 new tests. ✅ **b2 RUN 1 DONE 14:34 (~52p): 91.1% (112/123) — highest yet; 11 rejections: claim's own wording 5 · topic-without-finding 4 (was ~6/run; Venus 0, rating-only 0, trial-design 0) · arguable 2. Both Sweden causal elements kept "lower" by prompt alone (repair never fired), zero misreads on them.** ⚠️ WATCH: Sweden causation UNRESOLVED on both records — the pool lacked the Nature/UVA counterfactual sources this draw (pool churn vs the reworded element's lane queries — not separable from one run; if it holds on runs 2–3 it is a structure regression on the retrieval side). ✅ **b2 RUN 2 DONE 14:57 (~52p): 88.5% (108/122), 14 rejections: claim's own wording 4 · topic-without-finding 5 · reviewer error 2 · ⚠️ unstated-quantity question 3 in a NEW head ("How does the proportion … compare" — widen `unstated_quantity.py` after run 3, not mid-series). WATCH CLEARED: t06 causation DISPUTED with "lower" kept — run 1's unresolved was pool churn. t03 creatine got ZERO web sources (web_search timeout + four adapters 0 results in one window — network).** ✅ **b2 RUN 3 DONE 15:27 (`b2run3b`; the first attempt died on an environment event after one input, ~5p lost): 88.7% (102/115), 13 rejections — claim's own wording 4 · topic-without-finding 5 (incl. an IEA window that is a bare chart citation) · reviewer error 2 · arguable 2 · unstated-specificity 0 · Sweden misreads 0.** **BUILD-2 THREE-RUN READING (design doc §8): 89.4% blind (322/360; 91.1 / 88.5 / 88.7), adjudicated ≈94.2%; direction misreads 5 → 0; topic-without-finding 17 → 14; rate move 87.6 → 89.4 is p = 0.43 (kinds are the evidence, not the rate).** Remaining mass: the user's own wording 13 (not ours) · passage-level 14 (a window that is a citation / definition / badge; same-subject reading) · reviewer error 4 · one grounds question in an uncovered head 3 (detector WIDENED after run 3, `unstated_quantity.py`, 7 new tests) · arguable 4. **Owed, paid, ask: ONE corpus record (~£1.20) — the mapping prompt re-keyed every mapping cassette.** **BUILD 3 PREMISE CHECKED before approval (design doc §9, `tmp/verify-build3-2026-09-10.txt`): the "≈4 points, mechanical" claim was OVERSTATED — only 6 of 83 rejections carry a SAFE lexical signature (badge / chrome / citation / definition; 1 justified label at risk); 71 have none (mapper same-subject reading + reviewer). A window guard recovers ≈0.8 points blind, lifting adjudicated to ≈95%. Nothing left in the data closes the 95% blind bar without softening the user's words. **FOUNDER DREW THE LINE 2026-09-10 ~16:00: no build 3. Label axis FINAL for this push: 89.4% blind (three-run), ≈94–95% adjudicated; structure 8/10. Next: corpus record (running) → push → re-run the five outreach records on the deployed build → founder reads → sends. NEW SCOPE FROM THE FOUNDER: "literally hundreds of people to send emails out to, and associated marketing information — the outreach push is here" — a volume motion beside the 50-name bespoke plan; plan owed (segments, list source, sending infrastructure, collateral), NO fabricated addresses, PECR corporate-subscriber rule for UK cold email.** **RECIPIENT SWEEP 1 DONE 2026-09-10 ~15:50 — 667 validated rows in `audit/recipients/master.csv` (gitignored), zero rejections.** Six parallel collectors, every row citing the public page where its route was seen; six random email rows spot-checked verbatim on-page 6/6. By segment: journalists_uk 153 · newsletters 128 (UK 47, US 81) · legal_uk 90 · thinktank_uk 33 · comms_uk 29 · community_news 28 · libraries 28 · journalism_schools 23 · media_literacy 22 · civic 21 · dev_newsletters 32 · dev_directories 23 · dev_communities 16 · dev_press 14 · research_integrity 14 · osint 13. By route: email 227 · org_inbox 113 · bluesky 92 · substack 87 · contact_form 45 · press_office 31 · x 29 · submission_form 18 · linkedin 17 · github 8. Countries: UK 480 · US 141 · Ireland 25. 649/667 carry a dated current_topic. **55 rows carry a review flag** (address seen on a page other than the org's own domain; 19 Cloudflare-obfuscated addresses decoded from the page source — eyeball before sending; 14 journalist rows the collector marked weak). Blocked sources recorded per collector (Law Society, IFS, King's Fund, Guardian/BBC/FT/Times fetch-refused, ESU, CILIP…) — a second sweep with the founder's browser or a media database would add these. Tooling: `backend/scripts/merge_recipients.py` (evidenced routes only, no personal mailboxes, dedup on best route); methodology `audit/2026-09-10_mass_contact_methodology.md`; segments/infra/kit `audit/2026-09-10_outreach_volume_plan.md`. **VERIFIED on request (`audit/2026-09-10_measurement_verification.md`, recomputed from the raw CSVs): rates accurate; precision-word count 3/8 → 0/0/0 mechanically; the kind claim holds (26 of 284 labels before, ≤1 of 362 after, p = 4×10⁻⁹); BUT the pooled RATE move 83.8 → 87.6 is p = 0.17 — real by kind, not proven by rate. Found a new anomaly: a lone "5 metres" source disputed JWST's 6.5 m mirror on run 3 after the echo gate demoted three corroborating supports (2 vs 1 = the strict tie) — candidate fix: one erroneous source must not dispute a fact N independent primaries state.** (`tmp/astra-regrade.py --arm default` + `backend/scripts/review_labels.py`, ~£0.50 + ~2p per run). One run is noise; the claim is about the blind label rate (was 86.1% → 81.6%; the target is Astra's 95%). ✅ **The corpus re-record is DONE (2026-09-10 midday, `935af76`, ~£1.45): `170 ok / 5 warn / 8 fail / 3 unexercised` + 82CF drift, 9/10 claims at zero misses, 0001 clean for the first time** — README header canonical; the bench is trustworthy again. ⚠️ **The specificity prompts (`2957133`) are LIVE in production** — pushed beneath the UI commits by the record-surfaces session before any measurement run; the measurement now grades what users already get.
2. **Founder eyeballs the Evidence lens on his own phone and desktop** — the third cut was verified by Playwright at 1280 + iPhone 15 (`design/preview/mobile-2026-09-10/filters3-*`), not yet by his eye. If it still reads wrong, the SegmentedRow idiom is the fixed point; change spacing/labels inside it, do not start a fourth shape.
3. ⚠️ **FOUNDER RULE (2026-09-10, verbatim): "you MADE THE NO MORE WORK on the pipeline rule, NOT me. I want an 8/10 output BEFORE any sends go out."** "Stop pipeline work and get strangers" was Claude's sequencing, not a decision — struck. The gate is the internal 8/10 on BOTH axes (structure ≈ met; blind label rate 86–88% vs the 95% bar — SHORT). Sends stay HELD; the next build is whatever the three runs name (passage-level mislabels are the leading kind). `audit/OUTREACH.md` notes still stand for when the bar is met. ⚠️ The two 2026-09-07 commercial-assessment docs are STILL UNTRACKED — founder to read before they land.
4. Owed, small, unpaid: `UnknownElementCard` still renders `uncertainty` without the Fix 1 gate (amber box); `metadata.same_domain_as_source` unobserved through article mode; `OPENAI_API_KEY` dead locally; 82CF/5647 nondeterministic at extraction.
...
**What happened on 2026-09-09, in one paragraph.** The founder handed Claude the `codex/evidence-quality` branch (27 Codex commits implementing OpenAI "Astra"'s 6→8/10 plan, `tmp/tru8-hands-on/tru8-route-to-eight.md`). Three reviewers audited it; one HIGH defect (a second failed strengthening on one check violated the ledger's one-`refund`-per-check index) and 21 broken tests were fixed; steps 7–8 of the plan were built; Astra's 14 inputs were re-run and graded; the fixes the grading named were built; the corpus was re-recorded; **`main` was fast-forwarded to `247a6f8` and DEPLOYED (~15:53 UTC), verified on Railway** (health, `alembic current` = `text_provenance (head)`, new tables/column/ledger index present, MCP preflight 200). Two follow-up pushes: docs (`955f442`) and the record-identity/revision-history disclosures (`6527d66`, live on `/r/`). ~£5.50 of paid runs, every one founder-approved. Day record, in order: [takeover](2026-09-09_takeover_stabilise.md) → [quantitative guard](2026-09-09_quantitative_result_guard.md) → [same-study gate](2026-09-09_same_study_scope_gate.md) → [integrated run 3](2026-09-09_integrated_run3.md) → [PDF export](2026-09-09_pdf_passage_export.md) → [**Astra regrade**](2026-09-09_astra_regrade.md) (read this one).

**Where the product stands.**
- On Astra's 14 inputs, default configuration (what is live): **about 8/10 on structure and outcomes** — every input keeps what the user typed (extraction restore), both headline failures gone (Bank Rate 3.75% supported; sqlite.org challenges "cannot occur"), the invented trial refused, 14/14 complete under 90 s.
- On whether each source label is warranted by its passage: a blind AI reviewer (`backend/scripts/review_labels.py`, element + passage only, never the label) puts it at **86.1% then 81.6%** on two runs — **below Astra's 95%**. ±5 pts is retrieval noise; the KINDS are stable: **18 of the last 27 rejections are decomposition demanding specificity the claim never stated** (EV grounds questions asking for "total volume"/"what proportion"; words like "consistently/quantified/verified/completely"). Two mislabels have no mechanical form (inflation false-proposition support; Mercury-only comparison).
- Candidate configuration (`ENABLE_PASSAGE_MAPPING`, `ENABLE_STRUCTURED_EXTRACTION`): **OFF, and NOT ready** — over-scopes true day-scoped values to `context_only` (Bank Rate), binds passage quotes to the wrong element (JWST). Leave it.

**Founder rules set 2026-09-09 (do not re-open):** keep the concentration-cap deletion · **ask before EVERY paid run, with the estimated cost** (~5p per live check incl. search units; ~2p per blind review) · public "System interpretation" gated fail-closed · docs current BEFORE each commit · **no human reviewer will be paid or found — the blind AI review IS the label gate** · when the founder says "drift", stop explaining and ship the product fix the measurement named.

**▶▶ WHAT HAPPENS NEXT (in order)**
1. ✅ **Sign-in: founder confirmed 2026-09-10 — worked fine afterwards, no further 500s.** The one 500 seen right after the deploy is attributed to the deploy window / Clerk's hosted step (no 5xx in Railway HTTP logs, nothing in Sentry). Closed.
2. ✅ **BUILT 2026-09-10 (unmeasured): decomposition specificity** — `audit/2026-09-10_decomposition_specificity.md`. The specificity rule is on both decomposition prompts (`MATCH THE CLAIM'S OWN SPECIFICITY`; mechanism only where the claim asserts one) AND on the grounds question-builder; plus a mechanical backstop for the QUESTION form the adverb strip could not see — `app/utils/unstated_quantity.py` detects a question demanding a figure the claim never stated (narrow: the measured heads, never `how much`/`how many`; silent when the claim states a quantity) and routes it through the SAME repair call as compounds (`_repair_questions`, tagged items, one call, fail-safe, before the lock), disclosed at `metadata.grounds.specificity`. Flag `ENABLE_UNSTATED_QUANTITY_REPAIR` (default True). 37 new tests. ✅ **(a) Corpus RE-RECORDED 2026-09-10 (~£1.45, founder-approved in two steps): pass state `170 ok / 5 warn / 8 fail / 3 unexercised` + 82CF drift — 9/10 claims at ZERO misses, 0001 clean for the first time (19/0/0).** Two first-pass pools collapsed on network (30 ConnectTimeouts on 0001; fetch-deadline cancellations on 82CF) and were re-recorded singly; 82CF still drifts cross-process and its 18 MB re-record was reverted (README header canonical). Curated goldens restored by `tmp/restore-curated-goldens.py`; 018F recital re-pinned 2/1 → 2/2 from the observation. **Owed, paid, ask before each:** (b) `astra-regrade.py --arm default` + `review_labels.py` **three times** (~50p each) — read the KINDS before the rate.
3. **Then STOP pipeline work and go get strangers** (`audit/OUTREACH.md`; the 2026-09-07 commercial assessment §0). The five send notes still stand.
4. Owed, small, unpaid: `metadata.same_domain_as_source` was never observed through article mode (a test through phase 2); `OPENAI_API_KEY` is DEAD locally (401 confirmed) — only Gemini reviewers exist until a second-family key is set; the two flaky corpus claims (82CF, 5647) are nondeterministic across processes at extraction (root cause timeboxed).

**Bench (canonical: `backend/tests/replay_corpus/README.md` header):** `117 ok / 4 warn / 10 fail` + 3 drift — 82CF and 5647 at random; **018F re-keyed by the "specifically" strip, re-record deferred to after item 2.** ⚠️ `--update-golden` DROPS every curated invariant and tolerance-0 pin; restore from git and re-read pins against the observation (018F recital pin is now 2 refs / 1 element). ⚠️ `--record-missing` can converge in its own run and drift the next — prove determinism with a separate replay.

**Tooling that exists now:** `tmp/astra-regrade.py --arm default|candidate --out <dir> [--only …]` (Astra's 14 inputs, local DB, live retrieval, ~£0.50) · `tmp/astra-regrade-summary.py <dir>` (grader's view) · `backend/scripts/review_labels.py <run dir> <out dir>` (blind review, ~2p) · `backend/scripts/build_review_sheets.py` (human sheets, unused) · `backend/scripts/evaluate_result_fidelity.py` (candidate fixed-source eval) · `tmp/diag-cassette-misses.py` / `tmp/diag-extract-path.py` (bench forensics).
...
**Four ships, all on `main`, all pushed, all live-verified (`origin/main` = local at sign-off; backend health `53ec014`+, web deploys confirmed by DOM/PNG polls):**
| what | proof |
|---|---|
...
| **`d7439f5` — the public record's title is the WHOLE claim; the duplicate "Analysed" block is dropped when it only repeats it.** Founder, reading the shareable link: *"why are we only listing a percentage of the length of the title … Either the whole title, or remove it completely, as it is stated below."* Cause: the title is never stored — `get_public_check` derived it per request as the excerpt's first `.`-fragment cut at 70 chars + `...`, and `/r/` rendered that in the H1, tab title, share text and OG card. Now `text_check_title`: a single-claim check is titled by its claim, whole; multi-claim pasted text by its first sentence, whole, on a real boundary (decimals/initials don't end it — the first regex DID split "U.S.", the test caught it). Web hides the "Analysed" block when its text equals the heading (URL checks and long pasted text keep it). OG card clamp raised 150 → 200 (`8f95f77`, the console's own claim cap; 28px tier above 170) — at 150 the Scotland claim lost its last word ("…the £53 million…") with a spare line beneath it; verified locally against the prod API (`NEXT_PUBLIC_API_URL=https://api.trueight.com npm run dev` — the OG route fetches the backend, so a bare dev server renders "Unable to generate image"), then on production 123 s after the push: whole claim, three lines | 6 new unit tests + 34 public-endpoint tests, black clean; web 149/149, typecheck clean; **live 11:20 UTC, verified on TTE + Tapper: full claim once per page, no block, tab title whole**. Changes every record at once |
...
**Send-time discovery — NO EMAIL ADDRESSES, by design (founder, 2026-09-04: *"I have just realised I don't have email addresses?"*):** the roster carries channels under the contact map's *"No fabricated addresses"* rule and none of the five publishes one. Every public route checked this morning (Substack about pages; both Oxford profiles say only *"Website cebm.ox.ac.uk"*; Carbon Brief about + contact = web form, no addresses; New Statesman contact-us = functional desks only; henrytapper.com about/contact, agewage.com, the Parliament memo — nothing). **Each recipient now has a `Send via` line in the send sheet:** TTE + Seymour → Substack reply/DM (subscribe first) · Viglione + McSweeney → Carbon Brief contact form marked for the person, or Bluesky DM (`@giulianaviglione.bsky.social`, `@rtmcswee.carbonbrief.org`) · Tapper → comment under his post (**comment-length note written** — a letter does not fit a public comment) or X @henryhtapper. `henry@agewage.com` exists only on data brokers — never send to it as confirmed. Note for the machine: **verify the physical send route when a roster row is built, not on send day; write the note for the channel.** Oxford `*.ox.ac.uk` pages 403 WebFetch but serve to curl with a browser UA.
...
**Where the documents stand at sign-off (SOT check):** this register (what is open) · `audit/OUTREACH.md` ROUND 5 (the pressure pass + the routes finding — the outreach machine's record) · `audit/2026-08-21_send_sheet.md` (UNTRACKED, the five notes with `Send via` lines and the Tapper comment-length variant — the founder's working doc; **first-touch is NOT yet logged anywhere**) · `audit/2026-06-18_outreach_contact_map.md` (UNTRACKED roster; the five rows now carry "routes verified 2026-09-04, no published email") · memory `project_send_day_2026_09_04.md`. CLAUDE.md needed no change (bench state 185/1/13/2 still current; nothing structural shipped).

### ▶▶ WHAT HAPPENS NEXT (Monday 2026-09-07), in order:
1. **ASK: which of the five went, when, and by which route** (Substack reply/DM · Carbon Brief form · Bluesky DM · blog comment · X). Then **log first-touch in `audit/OUTREACH.md`** — a new "FIRST TOUCH" table under ROUND 5: recipient · date · route · record id · `?src=` tag. If the Tapper send went as a public comment, note that it is public. If any did NOT go, ask why — a route that failed (Substack DM refused, form bounced) is a roster fact to record.
2. **Read attribution honestly before reporting it — it measures SIGNUPS, not visits.** `?src=<tag>` is stored first-touch in localStorage (`web/lib/attribution.ts`), flushed once after sign-in, and lands write-once on `User.signup_source` only within **72 h of account creation** (`app/core/attribution.py`). Read it with `railway ssh` → `python -m scripts.signup_sources` (and `scripts.recent_activity` for WHAT they did); `railway run` cannot reach the prod DB. A recipient who reads the record and never signs up leaves NO trace here — and PostHog sees a `/r/` visit only if they accepted analytics cookies. Zero after a weekend is the expected reading, not a failure — the plan judges at 50 sends, not at 5.
3. **Founder-only, still open:** phone re-test of the homepage on a FRESH tab (iOS tap-zoom + compact cookie banner shipped 09-02; screenshot if it still opens scrolled) · the COMPARE hands-on pass (two comparisons a minute apart).
4. **After a few days of traffic:** front-door numbers (`claim_field_submit` vs `start_check_click`) → the Option B decision. Unchanged.
5. **Small follow-ups from today, no spend, when next in those files:** `UnknownElementCard` renders `uncertainty` without the Fix 1 gate — route it through `elementCaveatNote` · the 404 console error on record pages — IDENTIFIED 2026-09-10: Google's favicon service (`t1`/`t2.gstatic.com/faviconV2`) answers 404 for a source domain it has no icon for; `getFaviconUrl` + the `<img onError>` fallback already handle it (letter mark shown). Cosmetic, DevTools-only, no fix owed · header "N of M elements covered" vs GAPS "M of M have evidence" (counter semantics) · the outreach machine's rule: **verify the physical send route when a roster row is built, and write the note for that channel** — apply to round two (Macfarlane, Gid M-K, Ridley) before any note is drafted.
6. **Bench items, no spend:** must-fire precondition = "directionally referenced" · an out-of-history home for cassettes before the next re-record.
7. **Pipeline queue, still held behind the sends:** pool-quality gate Pieces 2 and 1 · echo detector blind spot (recitals of a COMMENTARY original — visible again today: TTE element 02 supported on five restatements, unflagged) · GAPS "well covered" honesty fix · MCP stream keepalive (~140 s) · item 7 stage 2 / item 8 Option A.

**How to verify a `/r/` page without the founder's Chrome (today's method, reusable):** `cd web && node scripts/verify-page.mjs <url> --select '[data-testid="element-caveat"]'` for the roster; for every lens, load `/r/<id>?view=librarian|seeker|cartographer|chronologist` headless and dump `main.innerText` (the scratchpad script pattern: `createRequire('C:/Users/projects/Tru8/web/package.json')` then `require('playwright')`); the OG card is `GET /api/og/social/<id>` — locally it needs `NEXT_PUBLIC_API_URL=https://api.trueight.com` or it renders "Unable to generate image". Full check ids live only in session transcripts if not written down — **write them down when a check is run.**

**Durable lessons written to memory today** (`project_send_day_2026_09_04.md`): a pressure pass covers only the states a record HAS — the first record in a new state renders a surface nobody has read · a lookup keyed by the wrong id fails silently into a fallback that looks like data · record full check ids at the moment a check is run · **an outreach row is not sendable until it says HOW the send physically happens, and the note is written for that channel** · a derived display field (the 70-char title) can be wrong everywhere at once and fixed everywhere at once — check whether a thing is stored before planning a migration.

**Spend today:** 0p. Docker's Postgres/Redis left running (harmless).
...
| commit | what | proof |
|---|---|---|
| `46efd58` | **Fix 1** — element caveats render on `/r/` as a grey `NOTE ·` row behind a fail-closed no-verdict gate | five live records read by DOM |
| `3ea7d06` | **iOS tap-zoom** — 16px floor on form fields for touch; hero field explicit | Playwright iPhone 15 |
| `cb6711a` | **Playwright** is the page-verification tool: `cd web && node scripts/verify-page.mjs <url> --device "iPhone 15"` | — |
| `0cb41b1` | **Compact phone cookie banner** — 164px strip, was ~55% of the screen | live iPhone 15 render |
| `adf252d` | **Build A** — the claim lane's lead query gets an UNWINDOWED twin (`ENABLE_CLAIM_LANE_UNWINDOWED_TWIN`) | `Query lanes … c0: 2` in prod logs |
| `3ccbdb8` | **Piece 3** — `RETRIEVAL_CACHE_VERSION` in the evidence-cache key; pd/pw claims cached 1 h | `[CACHE OK] … version=` in prod logs |
| `4286984` | **Fetch-phase deadline** (finished pages KEPT at 30 s) + **hosted-MCP idempotency both ends** | TTE run: `All tasks complete`, one check |

**Control arms for Build A — BOTH PASS:** Scotland `4be28cd1` (rebuttal in pool; £53m-forecast element now DISPUTED) and TTE `1bbf008a` (Heneghan/Jefferson's piece in pool as `context`; causation element UNRESOLVED; caveats say their point). It took four TTE attempts because three unrelated faults hid the result — a 24 h cache replay (`web_search=0`), a 45 s deadline that discarded every fetched page, and a hosted-MCP retry that ran and charged twice — all three fixed today. Record: `audit/2026-09-02_fetch_deadline_and_mcp_idempotency.md`, `…_claim_lane_unwindowed_twin_design.md`, `…_dissent_discovery_probe.md`.

**Send week:** the founder was sending THREE by hand this afternoon (Viglione `441144ac`, Seymour `fa08cff7`, McSweeney `e1e5de25` — all notes re-verified against the live pages and rewritten for Fix 1; sheet `audit/2026-08-21_send_sheet.md`, untracked). **The agent does not know which went — ask.** TTE and Tapper were HELD; they now have passing fresh records (`1bbf008a`, `4be28cd1`) but their notes are NOT yet rewritten.

**Decisions today:** headline grey KEPT · Option B = capped subsidy, measure first (front-door review §4B) · Brave is on the FREE plan (2,000/mo) · pool-quality gate SCOPED and its rule VERIFIED on nine records, Pieces 1+2 held behind the sends (`audit/2026-09-02_pool_quality_gate_scope.md`).

**Spend today:** 7 agent checks × 15p = £1.05 (one was the double-charge, one a cache replay) + <10p of search probes. Bench not re-recorded.

### ▶▶ WHAT HAPPENS NEXT, in order:
1. **Ask the founder which of the three were sent, and when** → log first-touch in `audit/OUTREACH.md`; watch `?src=` attribution.
2. **Rewrite TTE + Tapper notes against their fresh records** (`/r/1bbf008a…`, `/r/4be28cd1…`): the 08-31 pressure pass — rendered page (Playwright, `--select '[data-testid="element-caveat"]'`), every lens, the PDF text. No spend. Then send on the founder's word.
3. ✅ **BENCH RE-RECORDED 2026-09-03 (founder: "Go ahead and re-record the bench, if that is outstanding" — it was; last corpus commit 31 Aug, Build A landed 2 Sep). ~£0.80.** Four passes (`--record` 535 s → `--record-missing` nothing to patch → `--update-golden` → verification `--all`); 018F + 0005 merged by hand, pins kept. **New PASS state `185 ok / 1 warn / 13 fail / 2 unexercised` — and for the first time ZERO cassette misses on all 10 claims** (82CF, B4A3, 5647 — the "timing-flaky set" — all replay clean; the fetch-phase deadline appears to have made the fetch queue deterministic, unattributed). Every fail attributed in the README header. Three things learned on the way: **(a) 018F recital debt PAID** (3/3, the 2026-08-17 pin exactly). **(b) The interested-party red line is a comparator FALSE ALARM:** the pool DOES carry whitehouse.gov this time, but the mapper filed the claimant's own video `context` and the gate only re-labels directional refs — proved by replaying the cassette alone and reading the final claim map (subjects `['donald trump']`, `interested_party_match` fires on the URL). The precondition reads domain PRESENCE, coarser than the trigger; pins KEPT (3 visible fails), bench item owed: precondition = "directionally referenced" (needs a capture signal). **(c) A 32.5 MB Gallagher Re PDF on 5647 was refused by the 20 MiB guard live but the recorder stored it whole — a 30 MB gzip cassette headed for git history (pack already 505 MB; PDFs do not compress, truncating to 24 MiB still left 30 MB).** Fixed in `scripts/replay_bench/cassette.py`: `application/pdf` bodies over the guard are stored as a stub with `body_truncated_from` and replayed with that size as `content-length` and an empty body, so the guard refuses them by declared size — same skip path; only PDFs, because the HTML path has no size guard. 5647's cassette re-stubbed (36.6 → 5.9 MB), replay verified identical. Pinned by `tests/unit/replay_bench/test_cassette_body_cap.py` (cap ≥ `MAX_PDF_BYTES`); 75/75 bench unit tests. Pools moved both ways (82CF 5→13 sources, 0005 11→5 — the latter at the relevance scorer, 3 of 20 kept, not the fetch); the bench cannot attribute retrieval-size changes. **Durable: before believing a red gate line, read what the mapper did with the trap · check cassette sizes in `git diff --stat` before committing a re-record.**
4. **Founder phone re-test** on a fresh tab (iOS fix + banner); screenshot if it still opens scrolled.
5. **After a few days:** front-door numbers (`claim_field_submit` vs `start_check_click`) → Option B decision. Unchanged.
6. **Pipeline queue, held behind the sends:** pool-quality gate Pieces 2 (recovery searches the claim's own words unwindowed) and 1 (trigger reads quality) · echo detector blind spot (recitals of a COMMENTARY original — the Tapper miss) · GAPS lens "well covered" honesty fix · MCP stream keepalive (~140 s) · item 7 stage 2 / item 8 Option A.

**How to read any re-run honestly (learned the hard way today):** find the request id in Railway logs (`railway logs --lines 5000`) and check, in order — `[CACHE HIT]` (a replay, not a test) · `[RETRIEVE] Query lanes` (did the twin run) · `All tasks complete` vs `Tasks timed out after 45s` (was the web lane kept) · `[URL LEDGER] kept … <the critic's URL>` · `[PIPELINE METRICS] web_search=N` · exactly ONE `Starting phase 1 for check` for your call. A run that finishes in under 40 s never searched.

**Durable lessons written to memory today** (`project_fix1_element_caveat_2026_09_02.md`): a model-written free-text field means nothing until the prompt defines it — read the stored corpus for verdict language before rendering it · a guarantee written for "every lane's second query" silently misses a one-query lane · when a query's results are all blocklisted the lane contributes nothing · an idempotency guarantee must be checked where the expensive work starts, not where the money moves · a retrieval improvement can hide behind two unrelated faults — prove each layer in the logs · re-run once before sending to anyone likely to try the claim themselves · record the parameters of every probe.

---

### ▶▶ WHERE 2026-09-01 ENDED (history) — THE CLAIM FIELD IS THE FRONT DOOR, LIVE. Seven commits on main, all deployed and live-verified (`17a6843` front door · `8dd68cc` + `d2c14f3` security passes · `a02f8b6` footer row · `ce62d2d` direct-to-check · `988fd3e` section rhythm · `b0267ec` Clerk wait + mark-only interstitial). Founder ran claims on prod: "seems better".
...
### ▶▶ WHAT HAPPENS NEXT, in order:
1. **Founder-only, quick:** (a) ✅ **mobile eyeball DONE 2026-09-02 (iPhone 15) — two faults reported:** *"the webpage does not open at the very top"* and *"the text field clips off screen when clicked on, so the send button is half off screen."* **Cause of the second, certain from the CSS:** the root font is 14px on phones and the field used `text-base`, so the textarea rendered at 14px; **iOS Safari zooms the whole page when a focused control is under 16px**, and the zoom + offset persist on the tab after the keyboard closes — which very probably IS the first fault too (a reload of a zoomed tab does not open at the origin). **Fixed same hour:** hero field explicit `text-[16px] md:text-xl`, plus a global touch-only rule (`globals.css`, `hover:none and pointer:coarse and max-width:767px`) giving every input/textarea/select a 16px floor — the console form's fields had NO size class and inherited 14px, so they zoomed too. Desktop untouched. Ruled out for the first fault: no `autoFocus`, no mount-time `.focus()`/`scrollIntoView` on `/`, no `background-attachment`, mobile-nav focus trap runs only when open, `#record` anchor still exists. ⚠️ **Cannot be reproduced from here:** the site sends `frame-ancestors 'none'` + `X-Frame-Options: DENY` (so no iframe emulation) and this Chrome would not resize below ~1000px. **Founder to re-test on the phone after deploy; if the page still opens scrolled on a FRESH tab, send a screenshot.** ✅ **PLAYWRIGHT INSTALLED same day on the founder's go (`playwright` 1.62.1 as a `web` devDependency; Chromium + WebKit engines in the user cache).** `web/scripts/verify-page.mjs` (`npm run verify:page -- <url> [--device "iPhone 15"] [--engine webkit] [--focus css] [--select css] [--out png]`) renders any URL headless and prints scrollY, horizontal overflow, console errors and the submit button's rect; `verify-ui` skill now points at it, the Chrome extension is kept ONLY for Clerk-walled flows. **First proof, the live homepage as iPhone 15 (393×659) in BOTH engines after `3ea7d06`:** `scrollY 0` on load and after focusing the field, `horizontalOverflow 0`, textarea `16px`, go tile 54px wide ending 37px inside the right edge — screenshots `design/preview/2026-09-02_home_iphone15_{chromium,webkit}.png`. So the "does not open at the top" report is NOT reproduced by either engine: it is iOS-Safari-specific, consistent with the persisted tap-zoom theory. ⚠️ iOS tap-zoom and the keyboard viewport still do not reproduce anywhere but a phone — the founder's re-test remains the closing evidence.
   **🆕 FOUND BY THE FIRST PLAYWRIGHT SCREENSHOT (2026-09-02): on a FRESH phone visit the cookie banner covers the front door.** At iPhone 15 size the consent sheet (eyebrow + "We use cookies" + three-line paragraph + Cookie Policy link + three stacked buttons) occupies the lower ~55% of the first screen — the headline is cut in half, the lede and the claim field are hidden entirely; a stranger sees the mark and a cookie box (`design/preview/2026-09-02_home_iphone15_webkit.png`). Every outreach recipient on a phone meets this before the field. ✅ **BUILT same day on the founder's go ("Go on the compact cookie banner, build it"):** below `sm` the banner is a bottom strip — one sentence (*"Necessary cookies are always on; analytics only if you allow them. Cookie Policy."*), a 2-column grid **Reject analytics | Accept all**, "Manage preferences" as a small link beneath; eyebrow hidden, heading `sr-only`; the Manage panel and Save button unchanged; desktop (`sm+`) pixel-identical to before. The phone reject label is "Reject analytics" because analytics IS the only non-essential category (the Manage panel lists Necessary + Analytics only) — the same button carries "Reject non-essential" from `sm`. Consent logic untouched (`lib/consent` writes exactly as before; 4-test pin `components/legal/__tests__/cookie-consent.test.tsx`). **Measured locally with Playwright as iPhone 15: strip 164px tall of 659 (25%, was ~55%)** — mark, headline and lede fully visible; the field's top edge still sits ~35px under the strip on this viewport (the hero itself is ~600px tall on a phone; any bottom strip overlaps it), so one tap on either button reveals it. Screenshot `design/preview/2026-09-02_cookie_banner_compact_iphone15_local.png`. **✅ LIVE `0cb41b1`, verified on production with Playwright as iPhone 15: `[role=dialog]` 164px of 659, identical to local** (`design/preview/2026-09-02_cookie_banner_compact_iphone15_LIVE.png`). Desktop banner unchanged. (b) ✅ **headline grey `#B2B2BA` KEPT — founder decision 2026-09-02** ("more subtle; the black felt quite aggressive"). Contrast ~2.1:1 is a known, accepted trade for a decorative headline; do not re-raise. (c) confirm the post-sign-in path once from a signed-out browser (both halves were proved separately, the joined round trip was not).
...
3. **Option B — signed-out runs (the "search engine" experience for a stranger) — DECISION, not yet a build.** Gate written in advance: build when #2 shows submits dying at the modal, or the first strangers name sign-in as the friction. Prerequisites, none of which exist: per-IP + global daily budget with a graceful "sign in to keep going", Turnstile on the field (`turnstile-spin` skill), anonymous check ownership that attaches on sign-up, public-by-default for anonymous results, cross-user cache on public records. Design doc first; ~1 week. Review §4 option B. **2026-09-02: founder's money question ("I can't charge for signed-out runs") answered and LOGGED in the review doc §4B — capped subsidy, ~£11/month at 30 runs/day, £10–15/month ceiling if ever built; recommendation: do not build until item 2's numbers say so. Founder agreed to measure first.**
4. **Send week continues (item 5 below):** Viglione (re-run `441144ac`, note rewritten), Tapper, McSweeney (founder call: one 15p re-run first), TTE, Seymour. **✅ Fix 1 BUILT 2026-09-02 (founder: "Go on Fix 1, build it if you have properly design reviewed it") — design review `audit/2026-09-02_fix1_element_caveat_render_design.md`, which found what the 08-24 approval had not: the `uncertainty` field mixes genuine limits with adjudications ("The evidence consistently refutes this element" on the McSweeney re-run), so it ships through a FAIL-CLOSED lexical gate (`web/lib/element-caveat.ts`: sentinels · verdict words · intensified-evidence phrasing), rendered as a grey `NOTE ·` row in the shared `ElementList` roster (`/r/` + dashboard), verbatim or nothing, `line-clamp-2`, no colour. Corpus of 15 real stored sentences pinned as the test fixture (9 shown / 4 hidden / sentinels); 143 web tests green, typecheck clean. Rendered locally against the prod API before push: McSweeney 01+02 show, 03 ("refutes") hidden; Viglione both GWIS notes; Seymour nothing; TTE the OSR provenance line; Tapper all three (`design/preview/2026-09-02_fix1_element_caveat_local_e1e5de25.jpg`). Reported-not-built: F3 scope caveats render nowhere in the web app either; the prompt never defines the field (option D, pipeline work, bench re-record). ✅ LIVE on www.trueight.com within ~4 min of the push (`46efd58`); verified on all five live pages by DOM extraction (`data-testid="element-caveat"`): identical to the local render, screenshot `design/preview/2026-09-02_fix1_element_caveat_LIVE_e1e5de25.jpg`. **The four "PDF-only" sentences in the send sheet (TTE, Viglione, Tapper, McSweeney) were REWRITTEN against the live page the same hour**; Seymour's note is unchanged because both his caveats are adjudications the gate hides.**
   **⛔ TTE HELD 2026-09-02 — send FOUR, not five.** Founder challenged the TTE send (*"Aren't we suggesting their sourcing was thin, and then share our report, and that is thin also? How do we know Tru8 is covering a wide enough range of sources?"*) and paid 15p to find out: re-run `b0398fca` (same input, `max_age_hours=0`, today's pipeline). **Their piece is NOT in the new pool** — 16 reviewed / 9 kept: Surrey & Sussex ICB page (primary, new — a public statement of the 29%), NHS England planning framework (context), seven trade-press echoes; four elements all `supported`, zero challenges, no echo flag (no primary original on the support side), no OSR / Hansard / gov.uk / evaluation; all nine queries in the claim's own words. Gain: a true, visible (Fix 1) caveat on every element — *the pilot was ONE practice, Wealden Ridge Medical Partnership, not "GP practices"*. **Verdict: the original `11f54993` found their critique by topic overlap, not by looking for dissent; on this claim the tool gives a different answer each run, and Heneghan is the recipient most likely to paste the claim into the front door himself.** Held, not sent; original record + note kept in the sheet as fallback. **Pipeline finding, outranks item 8 in my view:** the 2026-08-20 discovery problem reconfirmed on the strongest-fit recipient — retrieval searches FOR the claim, never against it. Query-side fixes are ruled out (Phase D, 0/3); what is not yet tried is a **dissent lane fed by something other than web search ranking**.
   **▶ PROBED THE SAME AFTERNOON (founder: "run a search on serper.dev … there may be something else"): `audit/2026-09-02_dissent_discovery_probe.md`. THE ACTUAL MECHANISM IS THE DATE WINDOW, NOT THE WORDING.** With the pipeline's exact parameters the plain claim text finds TTE at rank 3 and the Scotland rebuttal at rank 3 — **unwindowed**. The claim lane runs that query ONCE with the inherited `pm` window (the F1-D3 hedge only unwindows a lane's 2nd query, and the synthesised claim lane has one), and Google's past-month window returns 4–6 facebook/instagram/linkedin links and nothing else — all on the blocklist, so **the claim lane contributed nothing on these claims**; the critic's presence then rode on element-lane churn. **Brave Goggles** (already in the chain; ⚠️ **FREE plan, 2,000 queries/month, 1/s — verified from the response headers after the founder queried my "£0 extra"**; B costs 1 Brave query per claim: free at today's volume, ~0.4p/query on Base at scale, and it shares the quota with Brave's fallback role) re-rank: a *generic* goggle (boost substack/ghost/medium, downrank facebook/instagram — no claimant knowledge) puts TTE at 2–3 and Scotland at 2; claimant-aware puts both at 1. **Dairy stays unfound by everything** (framing mismatch — the honest ceiling is 2/3). Serper has no new dissent feature (news/autocomplete/related/scholar all useless here); Kagi ($12/1k), DataForSEO backlinks (2p/request), Tavily/Perplexity all fail the no-Exa cost rule per check. **▶ BUILD A BUILT 2026-09-02 (founder: "lets give it a go … scoped properly, and design reviewed") — design review `audit/2026-09-02_claim_lane_unwindowed_twin_design.md`, B designed in its §6 and NOT built.** `ENABLE_CLAIM_LANE_UNWINDOWED_TWIN` (default True): in `_merge_element_plans` a wired claim lane's lead query gets a freshness=`none` copy at lane position 1 (pd/pw exempt like the hedge; skipped if the lead is already unwindowed); `query_twin_of` parallel array; in `_execute_planned_queries` the twin asks for a fixed 10 and is excluded from the lead's depth divisor (the lead keeps today's 13); `_allocate_fetch_budget` gives a twin an element lane's weight (claim lane 2/7 → 3/8 of the 40 slots, not 4/9). Self-supplied plans (Seeker) carry no key → unchanged. Tests: `test_claim_lane_unwindowed_twin.py` (12) + five seam pins updated with dated notes; 61/61 on the three seam suites. **Owed before it counts:** push (prod deploy) · **bench re-record** (`--all --update-golden`, ~£0.80 — every wired corpus claim now drifts) · **live control arm** (TTE + Scotland re-run after deploy, 2 × 15p, critic presence is the pass/fail) — both spends on the founder's go. TTE + Tapper wait for the control arm. The 20 Aug "plain query ❌" recorded no parameters — today's numbers supersede it. Recipients who click through to `/` land on the field; ⚠️ the `/r/` header CTA "Start a check" still links `/dashboard/new-check` (auth modal → console form) — the Phase A "`/r/` footer field" was never built.
   **✅ SEND-DAY RE-VERIFICATION DONE 2026-09-02 (founder: "just check what we have to send is correct").** All five notes checked against the live public payload, the visible DOM of the server-rendered `/r/` page (scripts stripped), and the four PDFs (downloaded, text-extracted — every caveat the notes attribute to "the downloadable record" is in it). **One fault, fixed in the send sheet: Seymour's note listed "Nature's fire-weather work" among the challengers — that paper is mapped `context`; the Nature item that challenges is the "breaking records" feature.** Now "Nature's coverage of the record-breaking seasons". TTE's accepted risk (a) re-quoted to the post-`b6309da` GAPS copy. No caveat text renders on any page (`uncertainty` renders only in `UnknownElementCard`, and no element here is unresolved), so every "PDF-only" sentence still holds. Lens clicks REPEATED later the same morning via the extension once Chrome was open — EVIDENCE lens ×5, GAPS lens ×2 with resolved-elements expanded: all hold, Seymour correction confirmed on the rendered ledger. **✅ McSweeney RE-RUN DONE 2026-09-02 on the founder's go (15p agent balance, ~1.2p real, 36 s) → `e1e5de25-f3c4-4cf8-a834-33bb5f2ef95f`. carbonbrief.org ENTERED the pool: `isFactcheck: true`, R·NEWS·01 reporting, `03 · challenges` — the Viglione-shape fault is closed on this record; the Reddit mirror sits unmapped. Shape changed 2→3 elements (pollution fell · mechanism · not-climate-change): 2 supported / 1 challenged 3-0-0, pool 8 primary (7 academic) · 2 reporting · 2 commentary, 41 reviewed. Note REWRITTEN against the rendered page (all three lenses, resolved-elements expanded) + PDF; grade B → B+. New honest seam named in the note: element 02 is worded "directly driven" and badged supported while its sources say amplifying — that caveat is PDF-only (Fix 1 again). The 53%/23% split renders nowhere a reader can see and is kept OUT of the note. Send sheet §5 is the record. **All five are now sendable; nothing owed from the founder before sending.**
...
6. **Housekeeping from today:** ✅ CLAUDE.md's backend start line corrected to `uvicorn main:app` (was `app.main:app`) at sign-off. Docker's Postgres/Redis containers were left running after verification (harmless; the bench needs them anyway).
