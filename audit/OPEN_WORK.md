...
2. **ONE bounded build: decomposition specificity.** A rule on `DECOMPOSITION_PROMPT`/`BATCH_DECOMPOSITION_PROMPT` AND on the grounds question-builder (`opinion_symmetry.apply_grounds_stage`): elements and questions must not demand figures, absolutes or qualities the claim did not state; `invented_precision.strip_invented_precision` stays as the mechanical backstop. **This re-keys every cassette** — do the corpus `--record` ONCE after it (018F is already owed; see bench below). Measure with `astra-regrade.py --arm default` + `review_labels.py` **three times** (~£1.50 total, ask) — one run is noise.
