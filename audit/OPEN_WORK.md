...
4. ▶ **NEXT — reach 8/10 on the default path, per the [regrade](2026-09-09_astra_regrade.md):** (a) re-verify the extraction restore on #2/#4/#6 (paid, ~15p — ask); (b) absence-of-evidence challenges (creatine: "not enough evidence" statements counted as `challenges`) — the candidate's scope review handles this, the default path needs a mechanical form; (c) ✅ question-shaped elements now read Addressed / Contested / Context only / Open on every badge and in the PDF (parity-locked table); (d) the wal.html 3.22.0 false gap is BY DESIGN: `exclude_source_url` excludes the submitted page's whole DOMAIN from evidence — **founder decision owed:** narrow it to the exact URL (self-corroboration then falls to the interested-party/echo gates, with a same-domain receipt) or keep it; (e) decomposition premises (Venus "has a measurable temperature"). **Candidate before activation:** narrow `fact_applicability` (a source stating the value with a date inside the window must count), fix per-element passage binding, then a fresh two-arm regrade. Then step 9. Founder-owned gates: human review of ≥200 directional relationships (≥95% justified target), 3–5 user usability tasks, and tracing Astra's three stalled checks (`10f35558…`, `6b70b074…`, `1eb6c8af…`) through Railway logs. Claude-owned: reliability runs across input paths and fault scenarios, review-timeout completeness (Venus final review hit its 25 s bound).
