...
class ProgressReporter:
    ...
    STAGE_MESSAGES = {
        "starting": "Initialising fact-check...",
    ...
    async def report_progress(
        ...
        Args:
            stage: Pipeline stage name (ingest, extract, retrieve, verify, judge, summary)
    ...
    async def events(
        ...
        logger.info(f"[SSE] Sending connected event for check {self.check_id}: {event_str[:100]}")
        yield event_str
        ...
        while True:
            try:
                ...
                if elapsed > max_duration_seconds:
                    logger.error(f"Pipeline exceeded max duration ({max_duration_seconds}s), forcing termination")
                ...
                if pipeline_task and pipeline_task.done():
                    ...
                    if exc:
                        ...
                        logger.info(f"Pipeline task completed successfully for {self.check_id}")
                        complete_event = {
                            "type": "completed",
                            "checkId": self.check_id,
                            "status": "completed",
                ...
                if pipeline_task and pipeline_task.done():
                    ...
                    if exc:
                        ...
                        # Pipeline completed successfully - send completion event
                        logger.info(f"Pipeline task completed during heartbeat for {self.check_id}")
                        complete_event = {
                            "type": "completed",
                            "checkId": self.check_id,
                            "status": "completed",
