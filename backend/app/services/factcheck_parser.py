...
This fixes issues where fact-check meta-claims confuse the evidence mapper
...
class FactCheckParser:
    ...
    async def parse_factcheck_evidence(
        ...
        for claim in claims:
            ...
            for evidence in evidence_list:
                ...
                if evidence.get("is_factcheck") or self._is_factcheck_domain(
                    evidence.get("url", "")
                ):
                    # Attempt to parse
    ...
    async def _parse_and_enrich(
        ...
        try:
            ...
            similarity = await self._calculate_claim_similarity(
                claim_text, parsed_data["target_claim"]
            )
            ...
            if similarity < settings.FACTCHECK_SIMILARITY_THRESHOLD:
                ...
                evidence["relevance_score"] = (
                    ...
                    * settings.FACTCHECK_LOW_RELEVANCE_PENALTY
                )
    ...
    async def _fetch_and_parse(self, url: str, parser) -> Optional[Dict[str, Any]]:
        """
    ...
    async def _calculate_claim_similarity(
        self, our_claim: str, their_claim: str
    ) -> float:
        """
