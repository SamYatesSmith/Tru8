...
class VectorStore:
        ...
        try:
            ...
            if settings.QDRANT_URL.startswith("http"):
                ...
                if settings.ENVIRONMENT != "development":
                    logger.warning(
                        "[VECTOR STORE] QDRANT_URL not set or invalid — "
                        "falling back to localhost:6333. This will fail in production."
                    )
    ...
    async def store_evidence_embeddings(
        ...
        for evidence in evidence_data:
            ...
            point = PointStruct(
                id=point_id,
                vector=(
                    evidence["embedding"].tolist()
                    if isinstance(evidence["embedding"], np.ndarray)
    ...
    async def get_evidence_by_ids(
        ...
        try:
            ...
            results = []
            for point in points:
                result = {"id": point.id, **point.payload}
                results.append(result)
        ...
        try:
            result = await self.client.delete(
                ...
                points_selector=rest.FilterSelector(
                    filter=Filter(
                        must=[
                            FieldCondition(key="check_id", match=Match(value=check_id))
                        ]
                    )
                ),
            )
...
async def store_check_evidence(
    check_id: str, claims_evidence: List[Dict[str, Any]]
) -> Dict[str, List[str]]:
...
async def retrieve_claim_evidence(
    claim_embedding: np.ndarray, claim_id: Optional[str] = None, limit: int = 5
) -> List[Dict[str, Any]]:
    """Retrieve evidence for a claim using vector similarity"""
    vector_store = await get_vector_store()

    return await vector_store.search_evidence_for_claim(
        claim_embedding=claim_embedding, claim_id=claim_id, limit=limit
    )
