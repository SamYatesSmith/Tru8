...
class SourceIndependenceChecker:
    ...
    def _load_ownership_database(self) -> Dict[str, Any]:
        ...
        try:
            db_path = os.path.join(
                os.path.dirname(__file__), "..", "data", "source_ownership.json"
            )
    ...
    def enrich_evidence(
        ...
        for ev in evidence_list:
            ...
            # Assign independence flag (check state media list before "Unknown" fallback)
            if "state" in parent.lower() or domain in self._get_state_media():
    ...
    def check_diversity(
        self, evidence_list: List[Dict[str, Any]], threshold: float = 0.6
    ) -> Tuple[float, bool]:
        """
        ...
        logger.info(
            f"Source diversity: {diversity_score:.2f} (threshold: {threshold}, "
    ...
    def get_diversity_metrics(
        ...
        for ev in evidence_list:
            ...
            parent_counts[parent] = parent_counts.get(parent, 0) + 1
            independence_counts[independence] = (
                independence_counts.get(independence, 0) + 1
            )
    ...
    def _get_state_media(self) -> List[str]:
        ...
        return ["rt.com", "sputniknews.com", "presstv.ir", "cgtn.com", "globaltimes.cn"]
