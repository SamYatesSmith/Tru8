...
class TestQueryAnswering:
    ...
    async def test_successful_query_answer_factual_question(
        ...
        mock_response.json = Mock(
            return_value={"choices": [{"message": {"content": mock_llm_response}}]}
        )
        ...
        with patch("app.pipeline.query_answer.httpx.AsyncClient") as mock_client_class:
            ...
            result = await answerer.answer_query(
                user_query, claims, evidence_by_claim, original_text
            )
        ...
        ), "Factual question with clear sources should have high confidence"
        assert "source_ids" in result, "Must cite sources"
        assert len(result["source_ids"]) >= 1, "Should cite at least one source"

    @pytest.mark.asyncio
    @pytest.mark.critical
    async def test_query_answer_with_source_citation(
        self, mock_search_api, mock_openai_client
    ):
        """
        ...
        for source in result["source_ids"]:
            assert (
                "source" in source or "url" in source
            ), "Source must include publisher or URL"
            if "url" in source:
                assert source["url"].startswith("http"), "URL should be valid"

    @pytest.mark.asyncio
    async def test_no_answer_available_low_quality_sources(
        self, mock_search_api, mock_openai_client
    ):
        """
        ...
        ), "Low-quality sources should yield low confidence"
    ...
    async def test_answer_conciseness(self, mock_search_api, mock_openai_client):
        ...
        assert (
            len(answer) <= 500
        ), "Answer should be concise (<500 chars for simple questions)"
    ...
    async def test_numerical_query_answer(self, mock_search_api, mock_openai_client):
        ...
        ), "Numerical query should return numbers"
    ...
    async def test_current_events_query(self, mock_search_api, mock_openai_client):
        ...
        ), "Recent authoritative sources should yield confidence"
        assert len(result.get("source_ids", [])) >= 1
        # May mention year/date in answer

    @pytest.mark.asyncio
    async def test_complex_query_multi_part_answer(
        self, mock_search_api, mock_openai_client
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_search_query_optimization_from_user_query(
        self, mock_search_api, mock_openai_client
    ):
        """
    ...
    async def test_llm_prompt_structure(self, mock_search_api, mock_openai_client):
        ...
        # Arrange
        answerer = QueryAnswerer()
        answerer.google_ai_api_key = ""  # Disable Google AI path to test OpenAI only
        ...
        mock_llm_response = json.dumps(
            {"answer": "Test answer", "confidence": 85, "sources_used": [0]}
        )
        ...
        assert (
            "concise" in prompt_text.lower()
            or "brief" in prompt_text.lower()
            or "short" in prompt_text.lower()
        )
    ...
    @pytest.mark.asyncio
    @pytest.mark.critical
    async def test_end_to_end_query_answer_pipeline(
        self, mock_search_api, mock_openai_client
    ):
        """
        ...
        ), "High-quality sources should yield high confidence"
        ...
        for source in result["source_ids"]:
            assert (
                "source" in source or "url" in source
            ), "Source must have source name or URL"
