    ...
    **kwargs,
    ...
    user = User(
        ...
        **{
            ...
            if k not in ["monthly_checks_used", "created_at"]
        },
    )
...
def create_test_check(
    ...
    **kwargs,
    ...
    check_data.update({k: v for k, v in kwargs.items() if k not in check_data})
...
def create_completed_check(
    session, user_id: str = "test_user_123", num_claims: int = 3, **kwargs
) -> Any:
    """
...
def create_test_claim(
    ...
    **kwargs,
    ...
    claim_data = {
        ...
        "claim_type": kwargs.get("claim_type", "empirical"),
        "is_time_sensitive": kwargs.get("is_time_sensitive", False),
    }

    # Add additional kwargs
    claim_data.update({k: v for k, v in kwargs.items() if k not in claim_data})
...
def create_test_evidence(
    ...
    **kwargs,
    ...
    # Add additional kwargs
    evidence_data.update({k: v for k, v in kwargs.items() if k not in evidence_data})
...
def create_conflicting_evidence_check(session, user_id: str = "test_user_123") -> Any:
    ...
    Created: 2025-11-03
    """
    check = create_test_check(session, user_id=user_id, status="completed")
...
def create_test_user_with_checks(
    session, clerk_id: str = "test_user_123", num_checks: int = 5
) -> Any:
    """
