...
| Q | **MERGED + DEPLOYED 2026-09-09 (`main` = `247a6f8`); default-path work continues** | **Evidence quality — OpenAI "Astra"'s 6→8/10 plan** (`tmp/tru8-hands-on/tru8-route-to-eight.md`; 14 hands-on checks, 10 findings, 9-PR sequence). Codex built steps 1–6 as 27 commits on `codex/evidence-quality`; Claude took it over 2026-09-09 after a three-reviewer audit, built steps 7–8 plus the regrade-named fixes, and **fast-forwarded `main` to `247a6f8` the same evening (Railway verified: health, `alembic current` = `text_provenance`, new tables/column/index present).** **Status + register: `audit/2026-09-07_evidence_quality_implementation.md` (latest checkpoint first) and `audit/OPEN_WORK.md` START HERE.** Candidate flags `ENABLE_PASSAGE_MAPPING` / `ENABLE_STRUCTURED_EXTRACTION` are OFF and stay OFF at merge; switching them on is a separate decision with its own paid evaluation (**ask before every paid run — founder rule 2026-09-09**). Founder-owned gates that no code can pass: human review of 200 directional relationships, a 3–5 user usability study, and tracing Astra's three stalled checks through Railway logs. **REGRADE 2026-09-09 (`audit/2026-09-09_astra_regrade.md`): default arm ≈ 7/10, candidate NOT ready** — Bank Rate 3.75% and the sqlite.org challenge are fixed, 14/14 completed; but 3 of 14 inputs lost their contested conjunct at EXTRACTION (now guarded by `extract.restore_single_thesis`: one claim back from a single-sentence text submission missing a content token → the user's sentence is restored verbatim, `restored_from` recorded). Candidate's `fact_applicability` gate over-scopes true day-scoped values to `context_only` and passage review binds quotes to the wrong element — fix both before any activation. **Same-evening default-path fixes (all founder-approved):** `absence_of_evidence` gate (default-on: "not enough evidence" is context, never a challenge; measured nulls keep theirs) · question-shaped elements read **Addressed / Contested / Context only / Open** (`QUESTION_STATE_LABELS`, web+PDF parity-locked) · **source exclusion is now the submitted PAGE, not its domain** (`retrieve._source_exclusion`; domain-mates tagged `metadata.same_domain_as_source`; `RETRIEVAL_CACHE_VERSION=2026-09-09`) · decomposition prompts forbid trivially-true prerequisite elements (re-keys every cassette). **FINAL REGRADE (late 2026-09-09): about 8/10 by Claude's grading on Astra's 14 inputs, default path** — the human-review / usability / stall-trace gate is still the founder's. Open: SELECT composite endpoint; a residual premise element on creatine + Venus (the prompt rule alone did not stop it); `same_domain_as_source` tag not observed through article mode. |
...
Phase 2 (30-100%):
  ...
  DECOMPOSE (45%)     → Claim → 1-5 elements (LLM call); each element tagged with scope_flags {geographic,universal} — mechanical scope-sensitivity tagger, F3 Phase A 2026-07-07 (app/utils/scope_sensitivity.py). Normative-hinted claims (flag ON): elements rebuilt as NEUTRAL open questions by the grounds stage (`opinion_symmetry.apply_grounds_stage` — value-predicate lock, on-subject, structural coverage; balance lives in retrieval+mapping, never forced route symmetry). **Elements are ATOMIC as of Phase 3a (`2d77e7b`, 2026-07-29, `ENABLE_ELEMENT_ATOMICITY` default True):** an element asking two questions at once was graded by whichever half the mapper read — and the trivially-satisfiable enumerative half ("What were the stated targets?") badged the whole element `supported` while the half bearing on the claim ("...and were they met?") went unchecked. Measured at **21.2% of grounds elements, 13.8% mixed-shape** before the fix; 0.0% after. A mechanical detector (`app/utils/atomicity.py` — splits only on a coordinator followed by a second interrogative head, so conjoined noun phrases like "efficacy and evidence base" stay ONE question) triggers a repair call that **rewrites 1→1, never splits** — splitting would blow the 5-element contract and drop the trailing, judgement-bearing conjunct. Repair runs **BEFORE the value-predicate lock** (a rewrite can collapse into the judgement itself). Design: `audit/2026-07-29_element_atomicity_design.md`. SOT: `audit/DECOUPLING_STATE.md`. **Decomposition specificity (2026-09-10, `audit/2026-09-10_decomposition_specificity.md`):** the 2026-09-09 blind label reviews put 18 of 27 rejections on decomposition demanding figures/absolutes/qualities the claim never stated. Both decomposition prompts and the grounds question-builder now carry a `MATCH THE CLAIM'S OWN SPECIFICITY` rule (mechanism only where the claim asserts one; an intervention is not a population behaviour). Mechanical backstops: assertion form → `invented_precision.strip_invented_precision` (adverbs); QUESTION form → `app/utils/unstated_quantity.py` (a question opening "what proportion / what is the total … volume" for a claim stating no quantity; deliberately never `how much`/`how many`) routed through the SAME repair call as compounds (`_repair_questions`, `ENABLE_UNSTATED_QUANTITY_REPAIR`), disclosed at `metadata.grounds.specificity`. ⚠️ Re-keys every cassette; measured 3× 2026-09-10: the targeted kind went 18/27 → 0/45 (p=4e-9), blind rate 87.5% mean (short of 95%; the rate move itself p=0.17). **Build 2 the same afternoon (`audit/2026-09-10_finding_not_topic_direction.md`):** `FINDING, NOT TOPIC` on both MAPPING prompts (a passage naming the subject with a related fact — a temperature without the comparison, a trial design without its result, a fact-check rating without the claim — is context, never support) · fact-check fallback snippet names the claim rated · causal elements KEEP the claim's direction (`direction_fidelity.py` detector + one fail-safe repair call on both decomposition paths, `ENABLE_DIRECTION_REPAIR`; `_is_causal_link` now matches "driver(s) of"). Unmeasured at commit; re-keys mapping cassettes
  ...
  CLASSIFY (75%)      → Tier/Type classification (batched LLM + heuristic fallback). **The domain concentration cap is GONE (Codex `6958152`, founder kept it 2026-09-09):** it relabelled over-concentrated primary/reporting items to commentary for pool share alone (sqlite.org's own WAL page became `commentary/analysis`), and tier feeds `_STATE_TIER_WEIGHTS`, so it changed states on live checks. Now `claim_map.metadata.source_concentration` DESCRIBES publisher concentration (independence `not_established`) and enforces nothing; pinned by `test_domain_concentration_cap.py::test_runner_no_longer_relabels_sources_by_domain_share`. ⚠️ The bench cannot see this change. **Also default-on since Track Q: `capture_text_provenance`** (pure Python, no LLM) retains ≤8×900-char element-selected windows + the original snippet per item in `Evidence.text_provenance` (JSONB, migration `text_provenance`), returned as `textProvenance` on owner AND public payloads; not signed. Post-classify (needs tiers): mechanical derivation annotation writes per-element basis sourcing notes — echo (a primary re-reported by ≥2 derivatives), F4 repetition (≥3 non-primary sources reciting the same wording across ≥2 ownership groups with NO primary anchor — sentence-shingle, `corroboration.annotate_repetition_clusters`, 2026-07-07), thin (commentary-only / single-outlet). Surfaced as grey no-verdict notes (dashboard + `/r/`); a flagged element is toppable via "Strengthen this claim". Parity-locked `support_structure.py` ↔ `support-structure.ts`
...
- **Fetch-phase deadline** (2026-09-02, `ENABLE_FETCH_PHASE_DEADLINE`, `RETRIEVE_FETCH_PHASE_TIMEOUT_S=30`): the per-claim 45 s wait (`RETRIEVE_CLAIM_TIMEOUT_S`) is all-or-nothing INSIDE the web task — when the 40 page fetches overran it, every fetched page was discarded (`[CLAIM 0] Tasks timed out after 45s: ['web_search']` → `0 web snippets`) and coverage recovery rebuilt a thin, month-windowed pool. Now the fetches run under their own 30 s deadline and **finished pages are kept**; stragglers are cancelled with URL-ledger receipts (`stage=fetch_deadline`). Tell: a run whose metrics say `web_search=N` but whose pool is all `ev-rec-*`/`ev-rpf-*` items had its web lane thrown away.
- **Hosted MCP idempotency** (2026-09-02): the streamable-HTTP stream dies at ~140 s, the client re-initialises and re-sends the tool call, and the server ran it again — one `tru8_check` = two charged checks. `tru8_mcp/tools.py` now sends `Idempotency-Key` (endpoint + payload + 10-min window) and `agent.py::_run_agent_pipeline` returns the ORIGINAL check when `charge()` hands back a transaction that already owns one (`X-Tru8-Idempotent-Replay: 1`, `chargedPence 0`). Record: `audit/2026-09-02_fetch_deadline_and_mcp_idempotency.md`.
...
| `backend/app/services/research_operations.py` | **Strengthening as ONE durable operation per check (Track Q step 4):** admission (idempotency key replay, 409 on a competing active op, `uq_research_active_check`) BEFORE debit; external work holds no DB transaction; `commit_result` re-captures the report under `FOR UPDATE` and refuses if the baseline hash moved; failure/timeout/cancel refunds ONCE via `_fail_locked`; a 30 s `reconciliation_loop` fails expired ops and never restarts paid work. ⚠️ Its refund is `kind=research_refund` (`KIND_RESEARCH_REFUND`), NOT `refund`: `ux_usage_events_check_kind` allows ONE `refund` row per check, so a second failed strengthening used to raise IntegrityError and strand the op (found + fixed 2026-09-09; `comparison.py`'s budget counts both kinds). `report_revisions.py` archives before/after snapshots (signed) — `archived_url` is EXCLUDED from the snapshot because Wayback writes it after completion. |
